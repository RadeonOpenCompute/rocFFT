...
template <typename T>
int transform(size_t *lengths, const size_t *inStrides,
              const size_t *outStrides, size_t batchSize,
              const size_t *inOffset, const size_t *outOffset,
              rocfft_array_type inArrType, rocfft_array_type outArrType,
              rocfft_result_placement place, rocfft_precision precision,
              rocfft_transform_type transformType, double scale, bool packed,
  ...
  case rocfft_array_type_complex_interleaved: {
    ...
    setupBuffers(device_id, size_of_input_buffers_in_bytes, 1,
                 input_device_buffers, size_of_output_buffers_in_bytes,
                 number_of_output_buffers, output_device_buffers);
    ...
    HIP_V_THROW(hipMemcpy(input_device_buffers[0], &input[0],
  ...
  case rocfft_array_type_complex_planar: {
    ...
    setupBuffers(device_id, size_of_input_buffers_in_bytes, 2,
                 input_device_buffers, size_of_output_buffers_in_bytes,
                 number_of_output_buffers, output_device_buffers);
    ...
    HIP_V_THROW(hipMemcpy(input_device_buffers[0], &real[0],
                          size_of_input_buffers_in_bytes,
                          hipMemcpyHostToDevice),
                "hipMemcpy failed");
    HIP_V_THROW(hipMemcpy(input_device_buffers[1], &imag[0],
                          size_of_input_buffers_in_bytes,
                          hipMemcpyHostToDevice),
                "hipMemcpy failed");
  } break;
  ...
  if ((place == rocfft_placement_inplace) && packed && (scale == 1.0) &&
      (inOffset[0] == 0) && (inOffset[1] == 0) && (outOffset[0] == 0) &&
      (outOffset[1] == 0)) {
    LIB_V_THROW(rocfft_plan_create(&plan, place, transformType, precision, dim,
                                   lengths, batchSize, NULL),
                "rocfft_plan_create failed");
  } else {
    LIB_V_THROW(rocfft_plan_description_create(&desc),
                "rocfft_plan_description_create failed");

    LIB_V_THROW(rocfft_plan_description_set_data_layout(
                    desc, inArrType, outArrType, inOffset, outOffset, 3,
                    strides, strides[3], 3, o_strides, o_strides[3]),
  ...
  if (workBufferSize) {
    HIP_V_THROW(hipMalloc(&workBuffer, workBufferSize),
  ...
  void **BuffersOut =
      (place == rocfft_placement_inplace) ? NULL : &output_device_buffers[0];

  LIB_V_THROW(rocfft_execution_info_create(&info),
  ...
  if (profile_count > 1) {
    ...
    double bytes = (double)batchSize * 2.0 *
                   (double)totalLen; // the scalar 2.0 is because read & write
    ...
    std::cout << "Execution wall time: " << 1000.0 * wtime << " ms"
              << std::endl;
    std::cout << "Execution gflops (wall time): " << (opsconst) / (1e9 * wtime)
              << std::endl;
    std::cout << "Bandwidth GB/s (wall time): " << (bytes) / (1e9 * wtime)
              << std::endl;
  }
  ...
  if ((place == rocfft_placement_notinplace) || (profile_count == 1)) {
    ...
    case rocfft_array_type_complex_interleaved: {
      ...
      if (place == rocfft_placement_inplace) {
        HIP_V_THROW(hipMemcpy(&output[0], input_device_buffers[0],
                              size_of_input_buffers_in_bytes,
                              hipMemcpyDeviceToHost),
                    "hipMemcpy failed");
      } else {
        HIP_V_THROW(hipMemcpy(&output[0], BuffersOut[0],
      ...
      for (unsigned i = 0; i < outfftBatchSize; ++i) {
        if (0 == (i % outfftVectorSizePadded)) {
          if (fabs(output[i].real() - outfftVectorSize) / outfftVectorSize >
              err_ratio) {
            checkflag = true;
            break;
          }
    ...
    case rocfft_array_type_complex_planar: {
      ...
      if (place == rocfft_placement_inplace) {
        HIP_V_THROW(hipMemcpy(&real[0], input_device_buffers[0],
                              size_of_input_buffers_in_bytes,
                              hipMemcpyDeviceToHost),
                    "hipMemcpy failed");
        HIP_V_THROW(hipMemcpy(&imag[0], input_device_buffers[1],
                              size_of_input_buffers_in_bytes,
                              hipMemcpyDeviceToHost),
                    "hipMemcpy failed");
      } else {
        HIP_V_THROW(hipMemcpy(&real[0], BuffersOut[0],
                              size_of_output_buffers_in_bytes,
                              hipMemcpyDeviceToHost),
                    "hipMemcpy failed");
        HIP_V_THROW(hipMemcpy(&imag[0], BuffersOut[1],
                              size_of_output_buffers_in_bytes,
                              hipMemcpyDeviceToHost),
                    "hipMemcpy failed");
      }
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    desc.add_options()("help,h", "produces this help message")(
        "version,v",
        "Print queryable version information from the rocfft library")(
        "info,i",
        "Print queryable information of all the runtimes and devices")(
        "printChosen",
        "Print queryable information of the selected runtime and device")(
    ...
    if (ioArrTypeSupport[inL][otL] == 0)
      throw std::runtime_error(
          "Invalid combination of Input/Output array type formats");

    bool packed = false;
    if ((iStrides[0] == 1) && (iStrides[1] == 0) && (iStrides[2] == 0) &&
        (iStrides[3] == 0) && (oStrides[0] == 1) && (oStrides[1] == 0) &&
        (oStrides[2] == 0) && (oStrides[3] == 0))
      packed = true;

    if ((transformType == rocfft_transform_type_complex_forward) ||
        (transformType ==
         rocfft_transform_type_complex_inverse)) // Complex-Complex cases
    {
    ...
    if (precision == rocfft_precision_single)
      tret = transform<float>(lengths, iStrides, oStrides, batchSize, iOffset,
                              oOffset, inArrType, outArrType, place, precision,
                              transformType, scale, packed, deviceId,
                              platformId, printInfo, profile_count);
    else
      tret = transform<double>(lengths, iStrides, oStrides, batchSize, iOffset,
                               oOffset, inArrType, outArrType, place, precision,
                               transformType, scale, packed, deviceId,
                               platformId, printInfo, profile_count);
  } catch (std::exception &e) {
    std::cerr << "rocfft error condition reported:" << std::endl
              << e.what() << std::endl;
    return 1;
  }
