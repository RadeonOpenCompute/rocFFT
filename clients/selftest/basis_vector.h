...
template <typename T, typename CT> class FftBasisVectorMixComplex {
...
public:
  FftBasisVectorMixComplex(size_t dim, size_t *length, size_t batch,
                           FftBasisChoice fbc)
      : mfbc(fbc), mix(NULL), ref(NULL), otp(NULL), dev(NULL) {
    B = batch;
  ...
  void Generate(CT **data, CT **mag, int dir = -1) {
    ...
    for (size_t h = 0; h < B; h++) {
      for (size_t q = 0; q < Q; q++) {
        ...
        ref[h * L + bvSpot[q][2] * N[1] * N[0] + bvSpot[q][1] * N[0] +
            bvSpot[q][0]][0] = c;
        ref[h * L + bvSpot[q][2] * N[1] * N[0] + bvSpot[q][1] * N[0] +
            bvSpot[q][0]][1] = d;
