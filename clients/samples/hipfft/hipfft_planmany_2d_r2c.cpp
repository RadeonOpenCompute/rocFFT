...
int main() {
  ...
  float checksum[3] = {
      0, 0, 0}; // 0 for input, 1 for fftw cpu result, 2 for hipfft gpu result
  int rank = 2;
  int n[2] = {5, 6};
  ...
  const fftwf_plan fftwPlan = fftwf_plan_many_dft_r2c(
      rank, n, howmany, cpu_in_out, inembed, istride, idist,
      (fftwf_complex *)cpu_in_out, onembed, ostride, odist, flags);
  ...
  for (int i = 0; i < howmany; i++) {
    ...
    for (int j = 0; j < n[0]; j++) {
      for (int k = 0; k < n1_padding_real_elements; k++) {
        int idx = i * n[0] * n1_padding_real_elements +
                  j * n1_padding_real_elements + k;
        std::cout << "[" << j << ", " << k << "]: (" << std::setprecision(2)
                  << std::setw(6) << cpu_in_out[idx] << ") ";
  ...
  result = hipfftPlanMany(&hipForwardPlan, rank, n, inembed, istride, idist,
                          onembed, ostride, odist, HIPFFT_R2C, howmany);
  ...
  result =
      hipfftExecR2C(hipForwardPlan, gpu_in_out, (hipfftComplex *)gpu_in_out);
  ...
  std::cout << "\n----------------------------------------------------\n"
            << "Final sum, input: " << checksum[0]
            << ", fftw result: " << checksum[1]
