...
int main() {
  ...
  const fftw_plan fftwPlan =
      fftw_plan_many_dft(rank, n, howmany, fftw_in, inembed, istride, idist,
                         fftw_out, onembed, ostride, odist, sign, flags);

  for (int i = 0; i < n[0]; i++)
    for (int j = 0; j < n[1]; j++) {
      fftw_in[i * n[1] + j][0] = fftw_in[i * n[1] + j][1] =
          (i * n[1] + j + 1 + 30) % 12;
  ...
  for (int i = 0; i < n[0]; i++) {
    for (int j = 0; j < n[1]; j++) {
      std::cout << "[" << i << ", " << j << "] : (" << setprecision(2)
                << setw(6) << fftw_in[i * n[1] + j][0] << ", "
                << setprecision(2) << setw(6) << fftw_in[i * n[1] + j][1]
                << ")  ";
    }
    std::cout << std::endl;
  }
  ...
  for (int i = 0; i < n[0]; i++) {
    for (int j = 0; j < n[1]; j++) {
      std::cout << "[" << i << ", " << j << "] : (" << setprecision(2)
                << setw(6) << fftw_out[i * n[1] + j][0] << ", "
                << setprecision(2) << setw(6) << fftw_out[i * n[1] + j][1]
                << ")  ";
    }
    std::cout << std::endl;
  }
  ...
  result = hipfftPlanMany(&hipPlan, rank, n, inembed, istride, idist, onembed,
