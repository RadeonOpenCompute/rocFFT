...
int main()
{
    ...
    const fftw_plan fftwPlan = fftw_plan_many_dft(rank, n, howmany,
        fftw_in, inembed, istride, idist,
    ...
    for (int i = 0; i < n[0]; i++) {
        for (int j = 0; j < n[1]; j++) {
            std::cout << "[" << i << ", " << j << "] : (" << setprecision(2) << setw(6) << fftw_in[i * n[1] + j][0]
    ...
    for (int i = 0; i < n[0]; i++) {
        for (int j = 0; j < n[1]; j++) {
            std::cout << "[" << i << ", " << j << "] : (" << setprecision(2) << setw(6) << fftw_out[i * n[1] + j][0]
    ...
    result = hipfftPlanMany(&hipPlan, rank, n,
