...
class accuracy_test_complex_2D
    : public ::TestWithParam<
          std::tuple<std::vector<size_t>, size_t, rocfft_result_placement,
                     rocfft_transform_type, size_t, data_pattern>> {
protected:
...
class accuracy_test_real_2D
    : public ::TestWithParam<
          std::tuple<std::vector<size_t>, size_t, data_pattern>> {
protected:
...
void normal_2D_complex_interleaved_to_complex_interleaved(
    std::vector<size_t> lengths, size_t batch,
    rocfft_result_placement placeness, rocfft_transform_type transform_type,
    size_t stride, data_pattern pattern) {
  size_t total_size = 1;
...
template <class T, class fftw_T>
void normal_2D_real_interleaved_to_hermitian_interleaved(
    std::vector<size_t> lengths, size_t batch,
    rocfft_result_placement placeness, rocfft_transform_type transform_type,
    size_t stride, data_pattern pattern) {
...
TEST_P(accuracy_test_real_2D,
       normal_2D_real_interleaved_to_hermitian_interleaved_single_precision) {
  std::vector<size_t> lengths = std::get<0>(GetParam());
  size_t batch = std::get<1>(GetParam());
...
TEST_P(accuracy_test_real_2D,
       normal_2D_real_interleaved_to_hermitian_interleaved_double_precision) {
  std::vector<size_t> lengths = std::get<0>(GetParam());
  size_t batch = std::get<1>(GetParam());
  data_pattern pattern = std::get<2>(GetParam());
  rocfft_result_placement placeness =
...
template <class T, class fftw_T>
void normal_2D_hermitian_interleaved_to_real_interleaved(
    std::vector<size_t> lengths, size_t batch,
    rocfft_result_placement placeness, rocfft_transform_type transform_type,
    size_t stride, data_pattern pattern) {
  std::vector<size_t> input_strides;
...
TEST_P(accuracy_test_real_2D,
       normal_2D_hermitian_interleaved_to_real_interleaved_single_precision) {
  std::vector<size_t> lengths = std::get<0>(GetParam());
  size_t batch = std::get<1>(GetParam());
  data_pattern pattern = std::get<2>(GetParam());
  rocfft_result_placement placeness =
...
TEST_P(accuracy_test_real_2D,
       normal_2D_hermitian_interleaved_to_real_interleaved_double_precision) {
  std::vector<size_t> lengths = std::get<0>(GetParam());
  size_t batch = std::get<1>(GetParam());
  data_pattern pattern = std::get<2>(GetParam());
  rocfft_result_placement placeness =
...
// COMPLEX TO COMPLEX
// *****************************************************
INSTANTIATE_TEST_CASE_P(rocfft_pow2_2D, accuracy_test_complex_2D,
...
INSTANTIATE_TEST_CASE_P(rocfft_pow3_2D, accuracy_test_complex_2D,
...
INSTANTIATE_TEST_CASE_P(rocfft_pow5_2D, accuracy_test_complex_2D,
...
// REAL  HERMITIAN
// *****************************************************
INSTANTIATE_TEST_CASE_P(rocfft_pow2_2D, accuracy_test_real_2D,
                        Combine(ValuesIn(pow2_range), ValuesIn(batch_range),
                                ValuesIn(pattern_range)));

INSTANTIATE_TEST_CASE_P(rocfft_pow3_2D, accuracy_test_real_2D,
                        Combine(ValuesIn(pow3_range), ValuesIn(batch_range),
                                ValuesIn(pattern_range)));

INSTANTIATE_TEST_CASE_P(rocfft_pow5_2D, accuracy_test_real_2D,
                        Combine(ValuesIn(pow5_range), ValuesIn(batch_range),
                                ValuesIn(pattern_range)));
