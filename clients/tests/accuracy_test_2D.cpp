...
#define POW2_RANGE                                                                                                                         \
    { 2, 4 }, { 8, 16 }, { 32, 128 }, { 256, 512 }, { 1024, 2048 }, { 4096, 8192 } /* malloc fail on 4GB Fiji Nano on the following size \ \
, {16384, 32768}, {65536, 131072}, {262144, 524288} */

#define POW3_RANGE \
    { 3, 9 }, { 27, 81 }, { 243, 729 }, { 2187, 6561 } /* malloc fail on 4GB Fiji Nano on the following size , {19683, 59049}, {177147, 531441} */
...
TEST_P(accuracy_test_complex_2D, normal_2D_complex_interleaved_to_complex_interleaved_single_precision)
{
    ...
    try {
        normal_2D_complex_interleaved_to_complex_interleaved<float, fftwf_complex>(lengths, batch, placeness, transform_type, stride, pattern);
...
TEST_P(accuracy_test_complex_2D, normal_2D_complex_interleaved_to_complex_interleaved_double_precision)
{
    ...
    try {
        normal_2D_complex_interleaved_to_complex_interleaved<double, fftw_complex>(lengths, batch, placeness, transform_type, stride, pattern);
    } catch (const std::exception& err) {
        handle_exception(err);
    }
}
...
TEST_P(accuracy_test_real_2D, normal_2D_real_interleaved_to_hermitian_interleaved_single_precision)
{
    ...
    try {
        normal_2D_real_interleaved_to_hermitian_interleaved<float, fftwf_complex>(lengths, batch, placeness, transform_type, stride, pattern);
    } catch (const std::exception& err) {
        handle_exception(err);
    }
}
...
TEST_P(accuracy_test_real_2D, normal_2D_real_interleaved_to_hermitian_interleaved_double_precision)
{
    ...
    try {
        normal_2D_real_interleaved_to_hermitian_interleaved<double, fftw_complex>(lengths, batch, placeness, transform_type, stride, pattern);
    } catch (const std::exception& err) {
        handle_exception(err);
    }
}
...
TEST_P(accuracy_test_real_2D, normal_2D_hermitian_interleaved_to_real_interleaved_single_precision)
{
    ...
    try {
        normal_2D_hermitian_interleaved_to_real_interleaved<float, fftwf_complex>(lengths, batch, placeness, transform_type, stride, pattern);
    } catch (const std::exception& err) {
        handle_exception(err);
    }
}
...
TEST_P(accuracy_test_real_2D, normal_2D_hermitian_interleaved_to_real_interleaved_double_precision)
{
    ...
    try {
        normal_2D_hermitian_interleaved_to_real_interleaved<double, fftw_complex>(lengths, batch, placeness, transform_type, stride, pattern);
    } catch (const std::exception& err) {
        handle_exception(err);
    }
}
