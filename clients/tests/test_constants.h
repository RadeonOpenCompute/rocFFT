...
#define POST_MULVAL                                                            \
  void mulval_post(void *output, uint outoffset, void *userdata,               \
                   float2 fftoutput)\n {                                       \
...
#define POST_MULVAL_PLANAR                                                     \
  void mulval_post(void *outputRe, void *outputIm, size_t outoffset,           \
                   void *userdata, float fftoutputRe, float fftoutputIm)\n {   \
    \n float scalar = *((float *)userdata + outoffset);                        \
...
#define POST_MULVAL_PLANAR_DP                                                  \
  void mulval_post(void *outputRe, void *outputIm, size_t outoffset,           \
                   void *userdata, double fftoutputRe, double fftoutputIm)\n { \
    \n double scalar = *((double *)userdata + outoffset);                      \
...
#define POST_MULVAL_LDS                                                        \
  void mulval_post(void *output, uint outoffset, void *userdata,               \
                   float2 fftoutput, __local void *localmem)\n {               \
    \n uint lid = get_local_id(0);                                             \
    \n __local float *lds;                                                     \
    \n if (outoffset < 16) \n {                                                \
...
template <typename T> inline size_t MaxLength2D(size_t rad) { return 0; }

template <> inline size_t MaxLength2D<float>(size_t rad) {
  switch (rad) {
  case 2:
    return large2;
  case 3:
...
template <> inline size_t MaxLength2D<double>(size_t rad) {
  switch (rad) {
  case 2:
    return dlarge2;
  case 3:
