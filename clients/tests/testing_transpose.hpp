...
template <typename T>
void print_matrix(vector<T> CPU_result, vector<T> GPU_result, size_t m,
                  size_t n, size_t lda) {
  for (int i = 0; i < m; i++)
    for (int j = 0; j < n; j++) {
      printf("matrix  col %d, row %d, CPU result=%f, GPU result=%f\n", i, j,
             (CPU_result[j + i * lda]).x, (GPU_result[j + i * lda]).x);
      printf("matrix  col %d, row %d, CPU result=%f, GPU result=%f\n", i, j,
             (CPU_result[j + i * lda]).y, (GPU_result[j + i * lda]).x);
    }
}

template <typename T>
void transpose_reference(size_t m, size_t n, T *A, size_t lda, T *B, size_t ldb,
                         size_t batch_count) {
  // transpose per batch
...
template <class T>
rocfft_status rocfft_transpose(size_t m, size_t n, T *A, size_t lda, T *B,
                               size_t ldb, size_t batch_count);

template <>
rocfft_status rocfft_transpose<float2>(size_t m, size_t n, float2 *A,
                                       size_t lda, float2 *B, size_t ldb,
                                       size_t batch_count) {
...
template <typename T>
rocfft_status testing_transpose(size_t M, size_t N, size_t lda, size_t ldb,
                                size_t batch_count) {
  ...
  HIP_V_THROW(
      hipMemcpy(hB.data(), dB, sizeof(T) * B_size, hipMemcpyDeviceToHost),
      "hipMemcpy failed");
  ...
  =================================================================== */
  if (status !=
      rocfft_status_invalid_dimensions) // only valid size, compare with cblas
  {
    transpose_reference<T>(M, N, hA.data(), lda, hB_copy.data(), ldb,
                           batch_count);
    ...
    for (int i = 0; i < batch_count; i++) {
      unit_check_general<T>(N, M, ldb, hB_copy.data() + M * ldb * i,
                            hB.data() + M * ldb * i);
    }
  }
