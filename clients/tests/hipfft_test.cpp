...
TEST(hipfftTest, CheckBufferSizeC2C) {
  ...
  EXPECT_TRUE(hipfftMakePlan1d(plan, n, HIPFFT_C2C, 1, &workSize) ==
              HIPFFT_SUCCESS);
...
TEST(hipfftTest, CheckBufferSizeR2C) {
  ...
  EXPECT_TRUE(hipfftMakePlan1d(plan, n, HIPFFT_R2C, 1, &workSize) ==
              HIPFFT_SUCCESS);
...
TEST(hipfftTest, CheckBufferSizeC2R) {
  ...
  EXPECT_TRUE(hipfftMakePlan1d(plan, n, HIPFFT_C2R, 1, &workSize) ==
              HIPFFT_SUCCESS);
  EXPECT_TRUE(workSize == 2 * n * sizeof(float));
...
TEST(hipfftTest, CheckBufferSizeD2Z) {
  ...
  EXPECT_TRUE(hipfftMakePlan1d(plan, n, HIPFFT_D2Z, batch, &workSize) ==
              HIPFFT_SUCCESS);
...
TEST(hipfftTest, CheckBufferSizeZ2D) {
  ...
  EXPECT_TRUE(hipfftMakePlan1d(plan, n, HIPFFT_Z2D, batch, &workSize) ==
              HIPFFT_SUCCESS);
  EXPECT_TRUE(workSize == 2 * n * batch * sizeof(double));
...
TEST(hipfftTest, RunR2C) {
  ...
  hipMemcpy(&out[0], d_out, (N / 2 + 1) * sizeof(hipfftComplex),
