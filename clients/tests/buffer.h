...
public:
  ...
  buffer(const size_t dimensions_in, const size_t *lengths_in,
         ...
         const size_t distance_in, const rocfft_array_type layout_in,
...
private:
  ...
  /*****************************************************/
  void initialize_strides(
      const size_t *strides_in) // TODO: stride may introduce bugs
  {
      ...
      if (mismatched_point_indices.size() != 0 && max_mismatches_output != 0 &&
        ...
        for (size_t i = 0;
          ...
          if (is_complex() || is_hermitian()) {
            std::cout << "+i*" << imag(mismatch.x, mismatch.y, mismatch.z,
                                       mismatch.batch);
          }
          std::cout << " / "
...
public:
  ...
  /*****************************************************/
  bool is_real() { return _layout == rocfft_array_type_real; }
    ...
    for (size_t batch = 0; batch < batch_size(); batch++) {
      for (size_t z = 0; z < length(dimz); z++) {
        for (size_t y = 0; y < length(dimy); y++) {
          for (size_t x = 0; x < length(dimx); x++) {
            int value =
                random_value() % (max_value + 1); // pluck a random value
            if (random_value() % 2)
            ...
            if (is_real()) {
              ...
              set_one_data_point(static_cast<T>(value), static_cast<T>(value),
                                 x, y, z, batch);
            }
          }
        }
