...
public:
    ...
    {
        if (this->is_real() != that.is_real() || this->is_hermitian() != that.is_hermitian() || this->is_complex() != that.is_complex()) {
            throw std::runtime_error("Buffers must be the same layout type for assignment operator");
        }

        if (this->_number_of_dimensions != that._number_of_dimensions || this->_batch_size != that._batch_size || this->_lengths != that._lengths) {
...
private:
        ...
        if (is_real()) {
            if (is_in_place()) {
                ...
                if (number_of_dimensions() == 1) {
                    ...
                    throw std::runtime_error("invalid dimensions in adjust_strides_and_distance_for_in_place_real_buffer()");
            } else
                throw std::runtime_error("this buffer is out of place and shouldn't be adjusting strides");
        } else
    ...
    bool points_are_about_equal(buffer<T>& other_buffer, size_t x, size_t y, size_t z, size_t batch)
    {
        if (is_real())
            ...
            return (floats_are_about_equal<T>(real(x, y, z, batch), other_buffer.real(x, y, z, batch)) && floats_are_about_equal<T>(imag(x, y, z, batch), other_buffer.imag(x, y, z, batch)));
        else
    ...
    size_t buffer_mismatches(buffer<T>& other_buffer, bool compare_method)
    {
        ...
        if (compare_method == pointwise_compare) {
            ...
            if (mismatched_point_indices.size() != 0 && max_mismatches_output != 0 && suppress_output == false) {
                std::cout << std::endl
                          << std::dec << mismatched_point_indices.size() << " of " << batch_size() * number_of_data_points_single_batch()
            ...
            for (size_t batch = 0; batch < batch_size(); batch++) {
                ...
                if (fabs(rms) > rmse_tolerance_this) {
                    if (suppress_output == false)
                        std::cout << std::endl
                                  << "RMSE accuracy judgement failure -- RMSE = " << std::dec << rms << ", maximum allowed RMSE = " << std::dec << rmse_tolerance_this << std::endl;
                    return 1;
                }
            }
...
public:
    ...
    bool operator==(buffer<T>& other_buffer)
    {
        ...
        if ((is_real() && !other_buffer.is_real()) || (!is_real() && other_buffer.is_real()) || (is_hermitian() && !other_buffer.is_hermitian()) || (!is_hermitian() && other_buffer.is_hermitian()) || (is_complex() && !other_buffer.is_complex()) || (!is_complex() && other_buffer.is_complex())) {
            return false;
        }
