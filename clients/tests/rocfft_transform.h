...
template <class T>
rocfft_status rocfft_plan_create_template(
    rocfft_plan *plan, rocfft_result_placement placement,
    rocfft_transform_type transform_type, size_t dimensions,
    const size_t *lengths, size_t number_of_transforms,
    const rocfft_plan_description description);

template <class T>
rocfft_status
rocfft_set_scale_template(const rocfft_plan_description description,
                          const T scale);

/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
template <class T> class rocfft {
private:
...
public:
  ...
  rocfft(const std::vector<size_t> lengths_in, const size_t batch_size_in,
         ...
         const rocfft_transform_type transform_type_in, const T scale_in) try
      : dim(lengths_in.size()),
        lengths(lengths_in),
        ...
        input(lengths_in.size(), lengths_in.data(), input_strides_in.data(),
              batch_size_in, input_distance_in, _input_layout, _placement),
        output(lengths_in.size(), lengths_in.data(), output_strides_in.data(),
               batch_size_in, output_distance_in, _output_layout, _placement) {
  ...
  void initialize_resource() {
    ...
    if (_placement != rocfft_placement_inplace) {
      if (is_planar(_output_layout)) {
        HIP_V_THROW(
            hipMalloc(&(output_device_buffers[0]), output.size_in_bytes()),
            "hipMalloc failed");
        HIP_V_THROW(
            hipMalloc(&(output_device_buffers[1]), output.size_in_bytes()),
            "hipMalloc failed");
  ...
  void initialize_plan() {
    ...
    if (input_strides[0] == 1 && output_strides[0] == 1 && scale == 1.0) {
      LIB_V_THROW(rocfft_plan_create_template<T>(
                      &plan, _placement, _transformation_direction, dim,
                      lengths.data(), batch_size, NULL),
...
// allocate the worksapce
  ...
  void set_layouts() {
    ...
    LIB_V_THROW(rocfft_plan_description_set_data_layout(
                    desc, _input_layout, _output_layout, 0, 0,
                    input_strides.size(), input_strides.data(),
                    input_strides[0] * lengths[0], output_strides.size(),
                    output_strides.data(), output_distance),
    ...
    LIB_V_THROW(rocfft_plan_create_template<T>(
                    &plan, _placement, _transformation_direction, dim,
                    lengths.data(), batch_size, desc),
                "rocfft_plan_create failed");
  }

  /*****************************************************/
  void transform(
      bool explicit_intermediate_buffer = use_explicit_intermediate_buffer) {
    ...
    {
      LIB_V_THROW(rocfft_execution_info_set_work_buffer(info, device_workspace,
                                                        device_workspace_size),
    ...
    void **BuffersOut = (_placement == rocfft_placement_inplace)
                            ? NULL
  ...
  /*****************************************************/
  void set_data_to_value(T real) { input.set_all_to_value(real); }

  /*****************************************************/
  void set_data_to_value(T real, T imag) { input.set_all_to_value(real, imag); }
  ...
  /*****************************************************/
  void set_data_to_random() { input.set_all_to_random(); }

  /*****************************************************/
  void set_data_to_buffer(buffer<T> other_buffer) { input = other_buffer; }

  /*****************************************************/
  buffer<T> &input_buffer() { return input; }

  /*****************************************************/
  buffer<T> &output_buffer() { return output; }
...
private:
  ...
  void write_local_input_buffer_to_gpu() {
    ...
    if (is_planar(_input_layout)) {
      HIP_V_THROW(hipMemcpy(input_device_buffers[0], input.real_ptr(),
                            input.size_in_bytes(), hipMemcpyHostToDevice),
                  "hipMemcpy failed");
      HIP_V_THROW(hipMemcpy(input_device_buffers[1], input.imag_ptr(),
                            input.size_in_bytes(), hipMemcpyHostToDevice),
                  "hipMemcpy failed");
    } else if (is_interleaved(_input_layout)) {
      HIP_V_THROW(hipMemcpy(input_device_buffers[0], input.interleaved_ptr(),
                            input.size_in_bytes(), hipMemcpyHostToDevice),
                  "hipMemcpy failed");
    } else if (is_real(_input_layout)) {
      HIP_V_THROW(hipMemcpy(input_device_buffers[0], input.real_ptr(),
  ...
  void read_gpu_result_to_output_buffer() {
    ...
    if (is_planar(_output_layout)) {
      HIP_V_THROW(hipMemcpy(output.real_ptr(), output_device_buffers[0],
                            output.size_in_bytes(), hipMemcpyDeviceToHost),
                  "hipMemcpy failed");
      HIP_V_THROW(hipMemcpy(output.imag_ptr(), output_device_buffers[1],
                            output.size_in_bytes(), hipMemcpyDeviceToHost),
                  "hipMemcpy failed");
    } else if (is_interleaved(_output_layout)) {
      HIP_V_THROW(hipMemcpy(output.interleaved_ptr(), output_device_buffers[0],
                            output.size_in_bytes(), hipMemcpyDeviceToHost),
                  "hipMemcpy failed");
    } else if (is_real(_output_layout)) {
      HIP_V_THROW(hipMemcpy(output.real_ptr(), output_device_buffers[0],
  ...
  void read_gpu_result_to_input_buffer() {
    ...
    if (is_planar(_input_layout)) {
      HIP_V_THROW(hipMemcpy(input.real_ptr(), input_device_buffers[0],
                            input.size_in_bytes(), hipMemcpyDeviceToHost),
                  "hipMemcpy failed");
      HIP_V_THROW(hipMemcpy(input.imag_ptr(), input_device_buffers[1],
                            input.size_in_bytes(), hipMemcpyDeviceToHost),
                  "hipMemcpy failed");
    } else if (is_interleaved(_input_layout)) {
      HIP_V_THROW(hipMemcpy(input.interleaved_ptr(), input_device_buffers[0],
                            input.size_in_bytes(), hipMemcpyDeviceToHost),
                  "hipMemcpy failed");
    } else if (is_real(_input_layout)) {
      HIP_V_THROW(hipMemcpy(input.real_ptr(), input_device_buffers[0],
