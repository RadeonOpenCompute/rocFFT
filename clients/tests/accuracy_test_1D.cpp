...
#define POW2_RANGE                                                             \
  2, 4, 8, 16, 32, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, \
...
#define MIX_RANGE                                                              \
  6, 10, 12, 15, 20, 30, 120, 150, 225, 240, 300, 486, 600, 900, 1250, 1500,   \
      1875, 2160, 2187, 2250, 2500, 3000, 4000, 12000, 24000, 72000

#define LARGE_RANGE                                                            \
  4096, 4050, 4000, 3888, 3840, 3750, 3645, 3600, 3456, 3375, 3240, 3200,      \
      3125, 3072, 3000, 2916, 2880, 2700, 2592, 2560, 2500, 2430, 2400, 2304,  \
      2250, 2187, 2160, \ 2048, 2025, 2000, 1944, 1920, 1875, 1800, 1728,      \
      1620, 1600, 1536, 1500, 1458, 1440, 1350, 1296, 1280, 1250, 1215, 1200,  \
      1152, 1125, 1080, 1024, 1000, 972, 960, \ 900, 864, 810, 800, 768, 750,  \
      729, 720, 675, 648, 640, 625, 600, 576, 540, 512, 500, 486, 480, 450,    \
      432, 405, 400, 384, 375, 360, 324, 320, 300, 288, 270, 256,  \ 250, 243, \
      240, 225, 216, 200, 192, 180, 162, 160, 150, 144, 135, 128, 125, 120,    \
      108, 100, 96, 90, 81, 80, 75, 72, 64, 60, 54, 50, 48, 45, 40, 36, 32,    \
      30, 27, 25,\ 24, 20, 18, 16, 15, 12, 10, 9, 8, 6, 5, 4, 3, 2, 1
...
static rocfft_result_placement placeness_range[] = {rocfft_placement_notinplace,
...
static std::vector<size_t> generate_random(size_t number_run) {
  ...
  for (size_t r = 0; r < number_run; r++) {
    ...
    i = (size_t)(
        rand() %
        RAND_MAX_NUMBER); // generate a integer number between [0, RAND_MAX-1]
    j = (size_t)(
        rand() %
        RAND_MAX_NUMBER); // generate a integer number between [0, RAND_MAX-1]
    k = (size_t)(
...
void normal_1D_complex_interleaved_to_complex_interleaved(
    size_t N, size_t batch, rocfft_result_placement placeness,
    rocfft_transform_type transform_type, size_t stride) {
  ...
  complex_to_complex<T, fftw_T>(pattern, transform_type, lengths, batch,
                                input_strides, output_strides, input_distance,
                                output_distance, in_array_type, out_array_type,
                                placeness);
  usleep(1e4);
}
...
template <class T, class fftw_T>
void normal_1D_real_interleaved_to_hermitian_interleaved(
    size_t N, size_t batch, rocfft_result_placement placeness,
    rocfft_transform_type transform_type, size_t stride) {
  std::vector<size_t> lengths;
  lengths.push_back(N);
  ...
  data_pattern pattern = sawtooth;
  real_to_hermitian<T, fftw_T>(
      pattern, transform_type, lengths, batch, input_strides, output_strides,
      input_distance, output_distance, in_array_type, out_array_type,
      rocfft_placement_notinplace); // must be non-inplace tranform

  usleep(1e4);
}

TEST_P(accuracy_test_real,
       normal_1D_real_interleaved_to_hermitian_interleaved_single_precision) {
  size_t N = std::get<0>(GetParam());
  size_t batch = std::get<1>(GetParam());
  rocfft_result_placement placeness =
      rocfft_placement_notinplace; // must be non-inplace
...
TEST_P(accuracy_test_real,
       normal_1D_real_interleaved_to_hermitian_interleaved_double_precision) {
  size_t N = std::get<0>(GetParam());
  size_t batch = std::get<1>(GetParam());
  rocfft_result_placement placeness =
      rocfft_placement_notinplace; // must be non-inplace
...
template <class T, class fftw_T>
void normal_1D_hermitian_interleaved_to_real_interleaved(
    size_t N, size_t batch, rocfft_result_placement placeness,
    rocfft_transform_type transform_type, size_t stride) {
  std::vector<size_t> lengths;
  lengths.push_back(N);
...
TEST_P(accuracy_test_real,
       normal_1D_hermitian_interleaved_to_real_interleaved_single_precision) {
  size_t N = std::get<0>(GetParam());
  size_t batch = std::get<1>(GetParam());
  rocfft_result_placement placeness =
      rocfft_placement_notinplace; // must be non-inplace
...
TEST_P(accuracy_test_real,
       normal_1D_hermitian_interleaved_to_real_interleaved_double_precision) {
  size_t N = std::get<0>(GetParam());
  size_t batch = std::get<1>(GetParam());
  rocfft_result_placement placeness =
      rocfft_placement_notinplace; // must be non-inplace
...
// COMPLEX TO COMPLEX
// *****************************************************
INSTANTIATE_TEST_CASE_P(rocfft_pow2_1D, accuracy_test_complex,
                        Combine(ValuesIn(pow2_range), ValuesIn(batch_range),
                                ValuesIn(placeness_range),
                                ValuesIn(transform_range),
                                ValuesIn(stride_range)));

INSTANTIATE_TEST_CASE_P(rocfft_pow3_1D, accuracy_test_complex,
                        Combine(ValuesIn(pow3_range), ValuesIn(batch_range),
                                ValuesIn(placeness_range),
                                ValuesIn(transform_range),
                                ValuesIn(stride_range)));

INSTANTIATE_TEST_CASE_P(rocfft_pow5_1D, accuracy_test_complex,
                        Combine(ValuesIn(pow5_range), ValuesIn(batch_range),
                                ValuesIn(placeness_range),
                                ValuesIn(transform_range),
                                ValuesIn(stride_range)));

INSTANTIATE_TEST_CASE_P(rocfft_pow_mix_1D, accuracy_test_complex,
                        Combine(ValuesIn(mix_range), ValuesIn(batch_range),
                                ValuesIn(placeness_range),
                                ValuesIn(transform_range),
                                ValuesIn(stride_range)));

INSTANTIATE_TEST_CASE_P(rocfft_pow_random_1D, accuracy_test_complex,
...
// REAL  HERMITIAN
// *****************************************************
INSTANTIATE_TEST_CASE_P(rocfft_pow2_1D, accuracy_test_real,
                        Combine(ValuesIn(pow2_range), ValuesIn(batch_range)));

INSTANTIATE_TEST_CASE_P(rocfft_pow3_1D, accuracy_test_real,
                        Combine(ValuesIn(pow3_range), ValuesIn(batch_range)));

INSTANTIATE_TEST_CASE_P(rocfft_pow5_1D, accuracy_test_real,
                        Combine(ValuesIn(pow5_range), ValuesIn(batch_range)));

INSTANTIATE_TEST_CASE_P(rocfft_pow_mix_1D, accuracy_test_real,
                        Combine(ValuesIn(mix_range), ValuesIn(batch_range)));
...
#define HUGE_TEST_MAKE(test_name, len, bat)                                    \
  template <class T, class fftw_T> void test_name() {                          \
    std::vector<size_t> lengths;                                               \
    ...
    complex_to_complex<T, fftw_T>(pattern, transform_type, lengths, batch,     \
                                  ...
                                  in_array_type, out_array_type, placeness);   \
  }
