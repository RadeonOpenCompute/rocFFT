...
void complex_to_complex(data_pattern pattern,
                        ...
                        rocfft_result_placement placeness, T scale = 1.0f) {

  rocfft<T> test_fft(lengths, batch, input_strides, output_strides,
                     input_distance, output_distance, in_array_type,
                     out_array_type, placeness, transform_type, scale);

  fftw<T, fftw_T> reference(lengths, batch, input_strides, output_strides,
                            placeness, c2c);
...
void hermitian_to_real(data_pattern pattern,
  ...
  fftw<T, fftw_T> data_maker(lengths, batch, output_strides, input_strides,
                             placeness, r2c);
