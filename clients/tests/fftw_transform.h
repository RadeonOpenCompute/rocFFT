...
public:
  ...
  void make_plan(int x, int y, int z, int num_dimensions, int batch_size,
                 ...
                 fftw_direction direction, fftw_transform_type type) {
    ...
    if (type == c2c) {
      plan = fftwf_plan_many_dft(
          ...
          output_ptr, NULL, output_strides[0],
          num_points_in_single_batch * output_strides[0], direction,
          FFTW_ESTIMATE);
    } else if (type == r2c) {
      plan = fftwf_plan_many_dft_r2c(
          ...
          output_ptr, NULL, 1, (x / 2 + 1) * y * z, FFTW_ESTIMATE);
    } else if (type == c2r) {
  ...
  fftw_wrapper(int x, int y, int z, int num_dimensions, int batch_size,
    ...
    make_plan(x, y, z, num_dimensions, batch_size, input_ptr, output_ptr,
              num_points_in_single_batch, input_strides, output_strides,
              direction, type);
  }
...
public:
  ...
  fftw(const std::vector<size_t> lengths_in, const size_t batch_size_in,
       ...
       const rocfft_result_placement placement_in, fftw_transform_type type_in)
      : _lengths(lengths_in), _batch_size(batch_size_in),
        input_strides(input_strides_in), output_strides(output_strides_in),
        _direction(fftw_direction::forward), _type(type_in),
        _input_layout(
            initialized_input_layout()) // chose interleaved layout artificially
        ,
        _output_layout(initialized_output_layout()),
        input(lengths_in.size(), lengths_in.data(), input_strides_in.data(),
              batch_size_in, tightly_packed_distance, _input_layout,
              rocfft_placement_notinplace) // FFTW always use outof place
                                           // transformation
        ,
        output(lengths_in.size(), lengths_in.data(), output_strides_in.data(),
               batch_size_in, tightly_packed_distance, _output_layout,
               rocfft_placement_notinplace),
        _forward_scale(1.0f), _backward_scale(1.0f),
        fftw_guts((int)_lengths[dimx], (int)_lengths[dimy], (int)_lengths[dimz],
                  ...
                  input_strides, output_strides, _direction, _type) {
    clear_data_buffer();
  }
  ...
  /*****************************************************/
  void set_data_to_value(T value) { input.set_all_to_value(value); }
