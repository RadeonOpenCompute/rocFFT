...
#define TRANSPOSE_CALL(NUM_Y, DRN, TWL, TTD)                                   \
  hipLaunchKernelGGL(HIP_KERNEL_NAME(transpose_var1<float2, DRN, TWL, TTD>),   \
                     ...
                     dim3(data->gridParam.tpb_x, data->gridParam.tpb_x), 0, 0, \
                     (float2 *)data->twiddles_large, (float2 *)data->bufIn[0], \
                     (float2 *)data->bufOut[0], NUM_Y, data->inStride[1],      \
                     data->outStride[1], data->iDist, data->oDist)
...
#define POW2_SMALL_OP_2_C(FUNCTION_NAME, KERN_NAME)                            \
  void FUNCTION_NAME(void *data_p, void *back_p) {                             \
    ...
    if (data->direction == -1)                                                 \
      hipLaunchKernelGGL(                                                      \
          HIP_KERNEL_NAME(KERN_NAME<float2, -1>), dim3(data->gridParam.b_x),   \
          dim3(data->gridParam.tpb_x), 0, 0, (float2 *)data->twiddles,         \
          (float2 *)data->bufIn[0], (float2 *)data->bufOut[0],                 \
          data->length[1], data->inStride[1], data->outStride[1], data->iDist, \
          data->oDist);                                                        \
    else                                                                       \
      hipLaunchKernelGGL(                                                      \
          HIP_KERNEL_NAME(KERN_NAME<float2, 1>), dim3(data->gridParam.b_x),    \
