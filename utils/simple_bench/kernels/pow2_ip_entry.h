...
__global__ void fft_2048_ip_d2_s1(T *twiddles, T *buffer, const ulong len,
                                  const ulong stride, const ulong dist) {
...
template <typename T, int dir>
__global__ void fft_4096_ip_d2_s1(T *twiddles, T *buffer, const ulong len,
                                  const ulong stride, const ulong dist) {
  uint me = hipThreadIdx_x;
  uint batch = hipBlockIdx_x;
