...
__device__ void fft_2048(T *twiddles, T *lwb_in, T *lwb_out,
                         real_type_t<T> *lds, const uint me,
                         const ulong stride_in, const ulong stride_out) {
  ...
  if (sb == SB_UNIT) {
    ...
    lwbv[me + 256] =
        lib_make_vector4<vector4_type_t<T>>(X1.x, X1.y, X5.x, X5.y);
    lwbv[me + 512] =
        lib_make_vector4<vector4_type_t<T>>(X2.x, X2.y, X6.x, X6.y);
    lwbv[me + 768] =
        lib_make_vector4<vector4_type_t<T>>(X3.x, X3.y, X7.x, X7.y);
  } else {
...
template <typename T, StrideBin sb, int dir>
__device__ void fft_4096(T *twiddles, T *lwb_in, T *lwb_out,
                         real_type_t<T> *lds, const uint me,
                         const ulong stride_in, const ulong stride_out) {
  T X0, X1, X2, X3, X4, X5, X6, X7;
  ...
  if (dir == -1) {
    FwdRad16<T>(&X0, &X1, &X2, &X3, &X4, &X5, &X6, &X7, &X8, &X9, &X10, &X11,
                &X12, &X13, &X14, &X15);
  } else {
    InvRad16<T>(&X0, &X1, &X2, &X3, &X4, &X5, &X6, &X7, &X8, &X9, &X10, &X11,
                &X12, &X13, &X14, &X15);
  }
