...
__global__ void transpose_var1(T *twiddles_large, T *pmComplexIn,
                               T *pmComplexOut, const ulong numGroupsY,
                               const ulong stride_i, const ulong stride_o,
                               const ulong dist_i, const ulong dist_o) {
  ...
  const Tile wgTileExtent = {localExtent.x * reShapeFactor,
  ...
  for (uint t = 0; t < wgUnroll; t++) {
    size_t xInd =
        localIndex.x + localExtent.x * (localIndex.y % wgTileExtent.y);
  ...
  for (uint t = 0; t < wgUnroll; t++) {
    ...
    size_t yInd =
        localIndex.y / groupingPerY + t * (wgTileExtent.y * transposeRatio);
    tmp = lds[yInd][xInd];
    size_t gInd = xInd + rowSizeinUnits * yInd;
    tileOut[gInd] = tmp;
  }
