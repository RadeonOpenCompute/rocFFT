...
rocfft_status rocfft_execution_info_set_work_buffer(rocfft_execution_info info,
  ...
  log_trace(__func__, "info", info, "work_buffer", work_buffer,
...
rocfft_status rocfft_execute(const rocfft_plan plan, void *in_buffer[],
  ...
  log_trace(__func__, "plan", plan, "in_buffer", in_buffer, "out_buffer",
            out_buffer, "info", info);
  ...
  if (execPlan.workBufSize > 0) {
    assert(info != nullptr);
    assert(info->workBufferSize >=
           (execPlan.workBufSize * 2 * plan->base_type_size));
  }
