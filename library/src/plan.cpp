...
// clang-format on
rocfft_status
rocfft_plan_description_set_scale_float(rocfft_plan_description description,
                                        float scale) {
...
rocfft_status
rocfft_plan_description_set_scale_double(rocfft_plan_description description,
                                         double scale) {
  description->scale = scale;
...
rocfft_status rocfft_plan_description_set_data_layout(
    ...
    rocfft_array_type out_array_type, const size_t *in_offsets,
    const size_t *out_offsets, size_t in_strides_size, const size_t *in_strides,
    size_t in_distance, size_t out_strides_size, const size_t *out_strides,
    size_t out_distance) {
  log_trace(__func__, "description", description, "in_array_type",
            in_array_type, "out_array_type", out_array_type, "in_offsets",
            in_offsets, "out_offsets", out_offsets, "in_strides_size",
            in_strides_size, "in_strides", in_strides, "in_distance",
            in_distance, "out_strides_size", out_strides_size, "out_strides",
            out_strides, "out_distance", out_distance);
...
rocfft_status
rocfft_plan_description_create(rocfft_plan_description *description) {
...
rocfft_status
rocfft_plan_description_destroy(rocfft_plan_description description) {
  log_trace(__func__, "description", description);
...
rocfft_status rocfft_plan_create_internal(
    rocfft_plan plan, rocfft_result_placement placement,
    ...
    const rocfft_plan_description description, bool dry_run) {
    ...
    case rocfft_transform_type_complex_inverse: {
      if (placement == rocfft_placement_inplace) {
        if (description->inArrayType == rocfft_array_type_complex_interleaved) {
          if (description->outArrayType !=
              rocfft_array_type_complex_interleaved)
            return rocfft_status_invalid_array_type;
        } else if (description->inArrayType ==
  ...
  if (!dry_run) {
    ...
    repo.CreatePlan(
        p); // add this plan into repo, incurs computation, see repo.cpp
  }
  return rocfft_status_success;
}
...
rocfft_status rocfft_plan_create(rocfft_plan *plan,
  ...
  log_trace(__func__, "plan", *plan, "placment", placement, "transform_type",
            transform_type, "precision", precision, "dimensions", dimensions,
            "lengths", log_len[0], log_len[1], log_len[2],
            "number_of_transforms", number_of_transforms, "description",
            description);

  std::stringstream ss;
  ss << "./rocfft-rider"
     << " -t " << transform_type << " -x " << log_len[0] << " -y " << log_len[1]
     << " -z " << log_len[2] << " -b " << number_of_transforms;
  if (placement == rocfft_placement_notinplace)
    ss << " -o ";
  if (precision == rocfft_precision_double)
    ss << " --double ";
  if (description != NULL)
    ss << " --isX " << description->inStrides[0] << " --isY "
       << description->inStrides[1] << " --isZ " << description->inStrides[2]
       << " --osX " << description->outStrides[0] << " --osY "
       << description->outStrides[1] << " --osZ " << description->outStrides[2]
       << " --scale " << description->scale << " --iOff0 "
       << description->inOffset[0] << " --iOff1 " << description->inOffset[1]
       << " --oOff0 " << description->outOffset[0] << " --oOff1 "
       << description->outOffset[1] << " --inArrType "
       << description->inArrayType << " --outArrType "
       << description->outArrayType;

  log_bench(ss.str());

  return rocfft_plan_create_internal(*plan, placement, transform_type,
                                     precision, dimensions, lengths,
                                     number_of_transforms, description, false);
}
...
rocfft_status rocfft_plan_get_work_buffer_size(const rocfft_plan plan,
  ...
  *size_in_bytes = execPlan.workBufSize * 2 * plan->base_type_size;
  ...
  log_trace(__func__, "plan", plan, "size_in_bytes ptr", size_in_bytes, "val",
            *size_in_bytes);
  return rocfft_status_success;
}
...
rocfft_status rocfft_plan_get_print(const rocfft_plan plan) {
  log_trace(__func__, "plan", plan);
  std::cout << std::endl;
  std::cout << "precision: " << ((plan->precision == rocfft_precision_single)
                                     ? "single"
                                     : "double")
            << std::endl;
...
void TreeNode::RecursiveBuildTree() {
  ...
  if ((parent == nullptr) && ((inArrayType == rocfft_array_type_real) ||
  ...
  case 1: {
    ...
    if (IsPo2(length[0])) // multiple kernels involving transpose
    {
      ...
      if (length[0] <= 262144 / PrecisionWidth(precision)) {
        ...
        scheme = (length[0] <= 65536 / PrecisionWidth(precision)) ? CS_L1D_CC
                                                                  : CS_L1D_CRT;
      } else {
        if (length[0] >
            (Large1DThreshold(precision) * Large1DThreshold(precision))) {
    ...
    {
      size_t supported[] = {
          4096, 4050, 4000, 3888, 3840, 3750, 3645, 3600, 3456, 3375, 3240,
          3200, 3125, 3072, 3000, 2916, 2880, 2700, 2592, 2560, 2500, 2430,
          2400, 2304, 2250, 2187, 2160, 2048, 2025, 2000, 1944, 1920, 1875,
          1800, 1728, 1620, 1600, 1536, 1500, 1458, 1440, 1350, 1296, 1280,
          1250, 1215, 1200, 1152, 1125, 1080, 1024, 1000, 972,  960,  900,
          864,  810,  800,  768,  750,  729,  720,  675,  648,  640,  625,
          600,  576,  540,  512,  500,  486,  480,  450,  432,  405,  400,
          384,  375,  360,  324,  320,  300,  288,  270,  256,  250,  243,
          240,  225,  216,  200,  192,  180,  162,  160,  150,  144,  135,
          128,  125,  120,  108,  100,  96,   90,   81,   80,   75,   72,
          64,   60,   54,   50,   48,   45,   40,   36,   32,   30,   27,
          25,   24,   20,   18,   16,   15,   12,   10,   9,    8,    6,
          5,    4,    3,    2,    1};
...
void TreeNode::TraverseTreeAssignBuffersLogicA(OperatingBuffer &flipIn,
  ...
  if (scheme == CS_REAL_TRANSFORM_USING_CMPLX) {
    ...
    if (cs) {
      ...
      assert(childNodes[1]->childNodes[cs - 1]->obOut ==
...
void TreeNode::TraverseTreeAssignPlacementsLogicA(rocfft_array_type rootIn,
  ...
  if (parent != nullptr) {
    placement = (obIn == obOut) ? rocfft_placement_inplace
  ...
  for (children_p = childNodes.begin(); children_p != childNodes.end();
       children_p++) {
...
void TreeNode::TraverseTreeAssignParamsLogicA() {
  ...
  case CS_L1D_TRTRT: {
    ...
    size_t smallerDim = biggerDim == childNodes[0]->length[0]
                            ? childNodes[0]->length[1]
                            : childNodes[0]->length[0];
    size_t padding = 0;
  ...
  case CS_2D_RTRT: {
    ...
    assert((trans2Plan->obOut == OB_USER_OUT) ||
           (trans2Plan->obOut == OB_TEMP_CMPLX_FOR_REAL) ||
  ...
  case CS_2D_STRAIGHT: {
    ...
    assert((rowPlan->obOut == OB_USER_OUT) ||
           (rowPlan->obOut == OB_TEMP_CMPLX_FOR_REAL) ||
    ...
    assert((colPlan->obOut == OB_USER_OUT) ||
           (colPlan->obOut == OB_TEMP_CMPLX_FOR_REAL) ||
  ...
  case CS_3D_RTRT: {
    ...
    size_t biggerDim = (length[0] * length[1]) > length[2]
                           ? (length[0] * length[1])
                           : length[2];
    size_t smallerDim = biggerDim == (length[0] * length[1])
                            ? length[2]
                            : (length[0] * length[1]);
    size_t padding = 0;
    if (((smallerDim % 64 == 0) || (biggerDim % 64 == 0)) && (biggerDim >= 512))
    ...
    assert((xyPlan->obOut == OB_USER_OUT) ||
           (xyPlan->obOut == OB_TEMP_CMPLX_FOR_REAL) ||
    ...
    trans1Plan->outStride.push_back(trans1Plan->length[0] *
                                    trans1Plan->outStride[1]);
  ...
  case CS_3D_STRAIGHT: {
    ...
    assert((zPlan->obOut == OB_USER_OUT) ||
           (zPlan->obOut == OB_TEMP_CMPLX_FOR_REAL) ||
...
void TreeNode::TraverseTreeCollectLeafsLogicA(std::vector<TreeNode *> &seq,
  ...
  if (childNodes.size() == 0) {
    ...
    if (obOut == OB_TEMP_CMPLX_FOR_REAL)
      cmplxForRealSize = (oDist * batch) > cmplxForRealSize ? (oDist * batch)
                                                            : cmplxForRealSize;
    if (obOut == OB_TEMP)
    ...
    for (children_p = childNodes.begin(); children_p != childNodes.end();
         children_p++) {
      (*children_p)
          ->TraverseTreeCollectLeafsLogicA(seq, tmpBufSize, cmplxForRealSize,
                                           blueSize, chirpSize);
    }
  }
}
...
void ProcessNode(ExecPlan &execPlan) {
  ...
  execPlan.rootPlan->TraverseTreeAssignPlacementsLogicA(
      execPlan.rootPlan->inArrayType, execPlan.rootPlan->outArrayType);
...
void PrintNode(std::ostream &os, const ExecPlan &execPlan) {
  ...
  os << "Work buffer ratio: " << (double)execPlan.workBufSize / (double)N
     << std::endl;
