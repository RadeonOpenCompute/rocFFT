...
template <typename T, size_t DIM_X, size_t DIM_Y, bool WITH_TWL, int TWL,
          int DIR, bool ALL>
__device__ void transpose_tile_device(const T *input, T *output, const size_t m,
                                      const size_t n, size_t gx, size_t gy,
...
#pragma unroll
    for (int i = 0; i < DIM_X; i += DIM_Y) {
      ...
      if (WITH_TWL) {
        if (TWL == 2) {
          if (DIR == -1) {
            TWIDDLE_STEP_MUL_FWD(TWLstep2, twiddles_large,
                                 (gx + tx1) * (gy + ty1 + i), tmp);
          } else {
            TWIDDLE_STEP_MUL_INV(TWLstep2, twiddles_large,
                                 (gx + tx1) * (gy + ty1 + i), tmp);
          }
        } else if (TWL == 3) {
          if (DIR == -1) {
            TWIDDLE_STEP_MUL_FWD(TWLstep3, twiddles_large,
                                 (gx + tx1) * (gy + ty1 + i), tmp);
          } else {
            TWIDDLE_STEP_MUL_INV(TWLstep3, twiddles_large,
                                 (gx + tx1) * (gy + ty1 + i), tmp);
          }
        } else if (TWL == 4) {
          if (DIR == -1) {
            TWIDDLE_STEP_MUL_FWD(TWLstep4, twiddles_large,
                                 (gx + tx1) * (gy + ty1 + i), tmp);
          } else {
            TWIDDLE_STEP_MUL_INV(TWLstep4, twiddles_large,
                                 (gx + tx1) * (gy + ty1 + i), tmp);
          }
        }
      }
...
__global__ void transpose_kernel2(const T *input, T *output, T *twiddles_large,
  ...
  if (ALL) {
    transpose_tile_device<T, DIM_X, DIM_Y, WITH_TWL, TWL, DIR, ALL>(
        input, output, DIM_X, DIM_X, hipBlockIdx_x * DIM_X,
        hipBlockIdx_y * DIM_X, ld_in, ld_out, twiddles_large);
  } else {
    size_t m = lengths[1];
    size_t n = lengths[0];
    size_t mm =
        min(m - hipBlockIdx_y * DIM_X, DIM_X); // the corner case along m
    size_t nn =
        min(n - hipBlockIdx_x * DIM_X, DIM_X); // the corner case along n
    transpose_tile_device<T, DIM_X, DIM_Y, WITH_TWL, TWL, DIR, ALL>(
        input, output, mm, nn, hipBlockIdx_x * DIM_X, hipBlockIdx_y * DIM_X,
        ld_in, ld_out, twiddles_large);
  }
}

template <typename T, size_t DIM_X, size_t DIM_Y, bool ALL>
__global__ void
transpose_kernel2_scheme(const T *input, T *output, T *twiddles_large,
                         size_t dim, size_t *lengths, size_t *stride_in,
                         size_t *stride_out, const size_t scheme) {
  ...
  if (ALL) {
    transpose_tile_device<T, DIM_X, DIM_Y, false, 0, 0, ALL>(
        input, output, DIM_X, DIM_X, hipBlockIdx_x * DIM_X,
        hipBlockIdx_y * DIM_X, ld_in, ld_out, twiddles_large);
  } else {
