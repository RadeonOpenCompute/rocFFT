...
template <> struct real_type<float4> { typedef float type; };

template <> struct real_type<double4> { typedef double type; };

template <> struct real_type<float2> { typedef float type; };

template <> struct real_type<double2> { typedef double type; };
...
template <class T> struct vector4_type;

template <> struct vector4_type<float2> { typedef float4 type; };

template <> struct vector4_type<double2> { typedef double4 type; };

template <class T> using vector4_type_t = typename vector4_type<T>::type;

/* example of using vector4_type_t */
// vector4_type_t<float2> float4_scalar;
// vector4_type_t<double2> double4_scalar;

template <rocfft_precision T> struct vector2_type;

template <> struct vector2_type<rocfft_precision_single> {
  typedef float2 type;
};

template <> struct vector2_type<rocfft_precision_double> {
...
template <typename T>
__device__ inline T lib_make_vector4(real_type_t<T> v0, real_type_t<T> v1,
                                     real_type_t<T> v2, real_type_t<T> v3);

template <>
__device__ inline float4 lib_make_vector4(float v0, float v1, float v2,
                                          float v3)
#ifdef __NVCC__
{
...
template <>
__device__ inline double4 lib_make_vector4(double v0, double v1, double v2,
                                           double v3)
#ifdef __NVCC__
{
...
template <typename T> __device__ T TWLstep1(T *twiddles, size_t u) {
  size_t j = u & 255;
  T result = twiddles[j];
  return result;
}

template <typename T> __device__ T TWLstep2(T *twiddles, size_t u) {
  size_t j = u & 255;
  T result = twiddles[j];
  u >>= 8;
  j = u & 255;
...
template <typename T> __device__ T TWLstep3(T *twiddles, size_t u) {
  size_t j = u & 255;
  T result = twiddles[j];
  u >>= 8;
  j = u & 255;
...
template <typename T> __device__ T TWLstep4(T *twiddles, size_t u) {
  size_t j = u & 255;
  T result = twiddles[j];
  u >>= 8;
  j = u & 255;
  ...
  j = u & 255;
  result = lib_make_vector2<T>(
      (result.x * twiddles[768 + j].x - result.y * twiddles[768 + j].y),
