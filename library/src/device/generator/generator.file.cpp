...
extern "C" rocfft_status initParams(FFTKernelGenKeyParams &params,
  ...
  params.fft_outputLayout = params.fft_inputLayout =
...
extern "C" void WriteButterflyToFile(std::string &str, int LEN) {
  ...
  if (!file.is_open()) {
    std::cout << "File: " << fileName << " could not be opened, exiting ...."
              << std::endl;
  }
...
=================================================================== */
extern "C" void
WriteCPUHeaders(std::vector<size_t> support_list,
...
extern "C" void write_cpu_function_small(std::vector<size_t> support_list,
                                         std::string precision, int group_num) {
  ...
  for (size_t j = 0; j < group_num; j++) {
    ...
    for (size_t i = i_start; i < i_end; i++) {
      ...
      std::string str_len = std::to_string(support_list[i]);
      str += "POWX_SMALL_GENERATOR( rocfft_internal_dfn_" +
             short_name_precision + "_ci_ci_stoc_" + str_len +
             ", fft_fwd_ip_len" + str_len + ", fft_back_ip_len" + str_len +
             ", fft_fwd_op_len" + str_len + ", fft_back_op_len" + str_len +
...
extern "C" void write_cpu_function_large(
    std::vector<std::tuple<size_t, ComputeScheme>> large1D_list,
  ...
  for (size_t i = 0; i < large1D_list.size(); i++) {
    ...
    if (scheme == CS_KERNEL_STOCKHAM_BLOCK_CC) {
      ...
      str += "POWX_LARGE_SBCC_GENERATOR( rocfft_internal_dfn_" +
             short_name_precision + "_op_ci_ci_sbcc_" + str_len +
             ", fft_fwd_op_len" + str_len + name_suffix + ", fft_back_op_len" +
             str_len + name_suffix + "," + complex_case_precision + ")\n";
    } else if (scheme == CS_KERNEL_STOCKHAM_BLOCK_RC) {
      name_suffix = "_sbrc";
      str += "#include \"rocfft_kernel_" + str_len + name_suffix + ".h\" \n";
      str += "POWX_LARGE_SBRC_GENERATOR( rocfft_internal_dfn_" +
             short_name_precision + "_op_ci_ci_sbrc_" + str_len +
             ", fft_fwd_op_len" + str_len + name_suffix + ", fft_back_op_len" +
             str_len + name_suffix + "," + complex_case_precision + ")\n";
    }
  }
...
=================================================================== */
extern "C" void AddCPUFunctionToPool(
