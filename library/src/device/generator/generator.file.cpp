...
extern "C" rocfft_status initParams(FFTKernelGenKeyParams& params, std::vector<size_t> fft_N, bool blockCompute, BlockComputeType blockComputeType)
{
    /* =====================================================================
...
=================================================================== */
extern "C" void WriteCPUHeaders(std::vector<size_t> support_list, std::vector<std::tuple<size_t, ComputeScheme> > large1D_list)
{
...
=================================================================== */
extern "C" void write_cpu_function_small(std::vector<size_t> support_list, std::string precision, int group_num)
{
    ...
    for (size_t j = 0; j < group_num; j++) {
        ...
        for (size_t i = i_start; i < i_end; i++) {
            ...
            std::string str_len = std::to_string(support_list[i]);
            str += "POWX_SMALL_GENERATOR( rocfft_internal_dfn_" + short_name_precision + "_ci_ci_stoc_" + str_len + ", fft_fwd_ip_len" + str_len + ", fft_back_ip_len" + str_len + ", fft_fwd_op_len" + str_len + ", fft_back_op_len" + str_len + ", " + complex_case_precision + ")\n";
        }
...
extern "C" void write_cpu_function_large(std::vector<std::tuple<size_t, ComputeScheme> > large1D_list, std::string precision)
{
    std::string str;
...
=================================================================== */
extern "C" void AddCPUFunctionToPool(std::vector<size_t> support_list, std::vector<std::tuple<size_t, ComputeScheme> > large1D_list)
{
    std::string str;
