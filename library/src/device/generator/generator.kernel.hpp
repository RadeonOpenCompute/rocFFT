...
template <rocfft_precision PR> class Kernel {
  ...
  */
  ...
  inline std::string OffsetCalcBlockCompute(const std::string offset_name1,
    ...
    loop += "\t\tcurrentLength *= (lengths[1]/" + std::to_string(blockWidth) +
            ");\n";
    loop += "\n";
    loop += "\t\t" + offset_name1 + " += (counter_mod/currentLength)*" +
            stride_name1 + "[i];\n";
    if (output == true)
  ...
  */
  ...
  inline std::string OffsetCalc(const std::string offset_name1,
                                const std::string stride_name1,
                                const std::string offset_name2,
                                const std::string stride_name2, bool output,
    ...
    */
    ...
    loop += "\tif(dim == 1){\n";
    loop +=
        "\t\t" + offset_name1 + " += counter_mod*" + stride_name1 + "[1];\n";
    if (output == true)
      loop +=
          "\t\t" + offset_name2 + " += counter_mod*" + stride_name2 + "[1];\n";
    loop += "\t}\n";

    loop += "\telse if(dim == 2){\n";
    loop += "\t\tint counter_1 = counter_mod / lengths[1];\n";
    loop += "\t\tint counter_mod_1 = counter_mod % lengths[1];\n";
    loop += "\t\t" + offset_name1 + " += counter_1*" + stride_name1 +
            "[2] + counter_mod_1*" + stride_name1 + "[1];\n";
    if (output == true)
    ...
    loop += "\t\t" + offset_name1 + " += counter_2*" + stride_name1 +
            "[3] + counter_1*" + stride_name1 + "[2] + counter_mod_1*" +
            stride_name1 + "[1];\n";
    if (output == true)
      loop += "\t\t" + offset_name2 + " += counter_2*" + stride_name2 +
              "[3] + counter_1*" + stride_name2 + "[2] + counter_mod_1*" +
              stride_name2 + "[1];\n";
    loop += "\t}\n";
...
public:
...

  {
    ...
    */
    ...
    if (c2r) {
      rcFull =
          ((params.fft_inputLayout == rocfft_array_type_complex_interleaved) ||
    ...
    if ((params.fft_MaxWorkGroupSize >= 256) && (nPasses != 0)) {
      ...
      assert(
          R ==
          1); // this has to be true for correct radix composition of the length
      numPasses = nPasses;
    } else {
  ...
  void GenerateKernel(std::string &str) {
    ...
    outReal =
        (params.fft_outputLayout == rocfft_array_type_real) ? true : false;
    ...
    for (size_t d = 0; d < 2; d++) {
      ...
      for (p = passes.begin(); p != passes.end(); p++) {
        ...
        p->GeneratePass(fwd, name_suffix, str, tw3Step, params.fft_twiddleFront,
                        inIlvd, outIlvd, inRl, outRl, ins, outs, s, gIn, gOut);
      }
    }
    ...
    for (int place = 0; place < 2; place++) {
      ...
      placeness =
          place ? rocfft_placement_notinplace : rocfft_placement_inplace;
      ...
      for (size_t d = 0; d < 2; d++) {
        ...
        */
        ...
        if (placeness == rocfft_placement_inplace) {
          ...
          if (blockCompute)
            str += OffsetCalcBlockCompute("ioOffset", "stride_in", "", "", true,
                                          false);
          else
          ...
          if (blockCompute) {
            str += OffsetCalcBlockCompute("iOffset", "stride_in", "oOffset",
                                          "stride_out", true, true);
          } else {
            str += OffsetCalc("iOffset", "stride_in", "oOffset", "stride_out",
                              true);
          }
        ...
        if (placeness == rocfft_placement_inplace) {
          if (inInterleaved) {
            ...
            inBuf =
                params.fft_hasPreCallback ? "gbRe, gbIm, " : "lwbRe, lwbIm, ";
                          ...
                          // would be overwritten
        {
          str += "\n\tfor(unsigned int t=0; t<";
          str +=
              std::to_string(blockWidth / (blockWGS / workGroupSizePerTrans));
          str += "; t++)\n\t{\n\n";
