...
class Kernel {
        ...
        bool output, bool rc_second_index = false)
    {
        ...
        if (output == true)
            loop += "\t\t" + offset_name2 + " += counter_1*" + stride_name2 + "[2] + counter_mod_1*" + stride_name2 + "[1];\n";
        loop += "\t}\n";
...
public:
...

    {
        ...
        if (r2c) {
            rcFull = ((params.fft_outputLayout == rocfft_array_type_complex_interleaved) || (params.fft_outputLayout == rocfft_array_type_complex_planar)) ? true : false;
        }
        if (c2r) {
            rcFull = ((params.fft_inputLayout == rocfft_array_type_complex_interleaved) || (params.fft_inputLayout == rocfft_array_type_complex_planar)) ? true : false;
        }
    ...
    void GenerateKernel(std::string& str)
    {
        ...
        inInterleaved = ((params.fft_inputLayout == rocfft_array_type_complex_interleaved) || (params.fft_inputLayout == rocfft_array_type_hermitian_interleaved)) ? true : false;
        outInterleaved = ((params.fft_outputLayout == rocfft_array_type_complex_interleaved) || (params.fft_outputLayout == rocfft_array_type_hermitian_interleaved)) ? true : false;
        ...
        for (size_t d = 0; d < 2; d++) {
            ...
            if (blockCompute && name_suffix == "_sbcc")
                str += "const T *twiddles_large, "; //the blockCompute BCT_C2C algorithm use one more twiddle parameter
            ...
            if (numPasses == 1) {
                ...
                str += "<T, sb>(twiddles, ";
                if (blockCompute && name_suffix == "_sbcc")
                    str += "twiddles_large, "; //the blockCompute BCT_C2C algorithm use one more twiddle parameter
            ...
            for (size_t d = 0; d < 2; d++) {
                ...
                if (placeness == rocfft_placement_inplace) {
                    ...
                    if (inInterleaved) {
                        //str += "const ";
                        str += r2Type;
                        str += " * __restrict__   gbIn, "; //has to remove const qualifier due to HIP on ROCM 1.4
                    } else {
                        str += rType;
