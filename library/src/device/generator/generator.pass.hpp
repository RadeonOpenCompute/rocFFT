...
// FFT pass generator
template <rocfft_precision PR> class Pass {
...
#define SR_COMP_BOTH 2 // real & imag
  ...
  void SweepRegs(size_t flag, bool fwd, bool interleaved, size_t stride,
                 size_t component, double scale, bool frontTwiddle,
                 const std::string &bufferRe, const std::string &bufferIm,
                 const std::string &offset, size_t regC, size_t numB,
                 size_t numPrev, std::string &passStr,
    ...
    if (numB && (numB % 2 == 0) && (regC == 1) && (numButterfly % 2 == 0) &&
        (algLS % 2 == 0) && (flag == SR_WRITE) && (nextPass == NULL) &&
        interleaved && (component == SR_COMP_BOTH) && linearRegs &&
        enableGrouping && !fft_doPostCallback) {
...
public:
  Pass(size_t positionVal, size_t lengthVal, size_t radixVal, size_t cnPerWIVal,
       ...
       bool realSpecialVal)
      : position(positionVal), length(lengthVal), radix(radixVal),
        cnPerWI(cnPerWIVal), algL(L), algLS(LS), algR(R),
        linearRegs(linearRegsVal), halfLds(halfLdsVal), r2c(r2cVal),
        c2r(c2rVal), rcFull(rcFullVal), rcSimple(rcSimpleVal),
        realSpecial(realSpecialVal), enableGrouping(true), numB1(0), numB2(0),
        numB4(0), nextPass(NULL), fft_doPreCallback(false),
  ...
  void GeneratePass(bool fwd, std::string name_suffix, std::string &passStr,
                    bool fft_3StepTwiddle, bool twiddleFront,
                    bool inInterleaved, bool outInterleaved, bool inReal,
                    bool outReal, size_t inStride, size_t outStride,
                    double scale, bool gIn = false, bool gOut = false) const {
    ...
    if (r2c) {
      if (position == 0) {
        passStr += "\n\tif(rw)\n\t{";
        SweepRegs(SR_READ, fwd, inInterleaved, inStride, SR_COMP_REAL, 1.0f,
                  false, bufferInRe, bufferInIm, "inOffset", 1, numB1, 0,
                  passStr);
        passStr += "\n\t}\n";
        ...
        if (rcSimple) {
          ...
          SweepRegs(SR_READ, fwd, inInterleaved, inStride, SR_COMP_IMAG, 1.0f,
                    false, bufferInRe2, bufferInIm2, "inOffset", 1, numB1, 0,
                    passStr);
      ...
      if (position == 0) {
        ...
        passStr += "\n\n\t__syncthreads();\n";
        SweepRegs(SR_READ, fwd, outInterleaved, processBufStride, SR_COMP_REAL,
                  1.0f, false, processBufRe, processBufIm, processBufOffset, 1,
                  numB1, 0, passStr, false, oddp);
        passStr += "\n\n\t__syncthreads();\n";
        ...
        if (length > 1) {
          if (!fft_doPreCallback) {
            ...
            SweepRegsRC(SR_READ, fwd, inInterleaved, inStride, SR_COMP_IMAG,
                        1.0f, false, false, false, bufferInIm, bufferInIm,
                        "inOffset", passStr);
            passStr += "\n\t}\n";

            passStr += "\n\tif(rw > 1)\n\t{";
            SweepRegsRC(SR_READ, fwd, inInterleaved, inStride, SR_COMP_IMAG,
                        1.0f, false, true, false, bufferInRe2, bufferInRe2,
                        "inOffset", passStr);
            passStr += "\n\t}\n\telse\n\t{";
            SweepRegsRC(SR_READ, fwd, inInterleaved, inStride, SR_COMP_IMAG,
                        1.0f, true, true, false, bufferInRe2, bufferInRe2,
                        "inOffset", passStr);
            passStr += "\n\t}";
        ...
        passStr += "\n\n\t__syncthreads();\n";
        SweepRegs(SR_READ, fwd, outInterleaved, processBufStride, SR_COMP_IMAG,
                  1.0f, false, processBufRe, processBufIm, processBufOffset, 1,
                  numB1, 0, passStr);
        passStr += "\n\n\t__syncthreads();\n";
      }
      ...
      if ((!halfLds) || (halfLds && (position == 0))) {
        ...
        passStr += "\n\tif(rw)\n\t{";
        SweepRegs(SR_READ, fwd, inInterleaved, inStride, SR_COMP_BOTH, 1.0f,
                  false, bufferInRe, bufferInIm, "inOffset", 1, numB1, 0,
                  passStr, isPrecallVector);
        SweepRegs(SR_READ, fwd, inInterleaved, inStride, SR_COMP_BOTH, 1.0f,
                  false, bufferInRe, bufferInIm, "inOffset", 2, numB2, numB1,
                  passStr, isPrecallVector);
        SweepRegs(SR_READ, fwd, inInterleaved, inStride, SR_COMP_BOTH, 1.0f,
                  false, bufferInRe, bufferInIm, "inOffset", 4, numB4,
    ...
    if (fft_3StepTwiddle && twiddleFront) {
      tw3Done = true;
      if (linearRegs) {
        SweepRegs(SR_TWMUL_3STEP, fwd, false, 1, SR_COMP_BOTH, 1.0f, true,
    ...
    if ((position > 0) && (radix > 1)) {
      SweepRegs(SR_TWMUL, fwd, false, 1, SR_COMP_BOTH, 1.0f, false, bufferInRe,
                bufferInIm, "", 1, numB1, 0, passStr);
      SweepRegs(SR_TWMUL, fwd, false, 1, SR_COMP_BOTH, 1.0f, false, bufferInRe,
                bufferInIm, "", 2, numB2, numB1, passStr);
      SweepRegs(SR_TWMUL, fwd, false, 1, SR_COMP_BOTH, 1.0f, false, bufferInRe,
                bufferInIm, "", 4, numB4, 2 * numB2 + numB1, passStr);
    }
    ...
    if (fft_3StepTwiddle && !tw3Done) {
      ...
      if (linearRegs) {
        SweepRegs(SR_TWMUL_3STEP, fwd, false, 1, SR_COMP_BOTH, 1.0f, false,
                  bufferInRe, bufferInIm, "", 1, numB1, 0, passStr);
      } else {
        SweepRegs(SR_TWMUL_3STEP, fwd, false, 1, SR_COMP_BOTH, 1.0f, false,
                  bufferInRe, bufferInIm, "", 1, numB1, 0, passStr);
    ...
    if (halfLds) {
      ...
      if (nextPass == NULL) // last pass
      {
        if (r2c && !rcSimple) {
          if (!singlePass) {
            SweepRegs(SR_WRITE, fwd, inInterleaved, inStride, SR_COMP_REAL,
                      1.0f, false, bufferInRe, bufferInIm, "inOffset", 1, numB1,
                      0, passStr);
            passStr += "\n\n\t__syncthreads();\n";
            SweepRegsRC(SR_READ, fwd, inInterleaved, inStride, SR_COMP_REAL,
            ...
            SweepRegs(SR_WRITE, fwd, inInterleaved, inStride, SR_COMP_IMAG,
                      1.0f, false, bufferInRe, bufferInIm, "inOffset", 1, numB1,
                      0, passStr);
            passStr += "\n\n\t__syncthreads();\n";
            SweepRegsRC(SR_READ, fwd, inInterleaved, inStride, SR_COMP_IMAG,
          ...
          passStr += "\n\tif(rw)\n\t{";
          SweepRegs(SR_WRITE, fwd, outInterleaved, outStride, SR_COMP_REAL,
                    scale, false, bufferOutRe, bufferOutIm, "outOffset", 1,
                    numB1, 0, passStr);
          passStr += "\n\t}\n";
          ...
          if (!rcSimple) {
            ...
            passStr += "\n\tif(rw > 1)\n\t{";
            SweepRegs(SR_WRITE, fwd, outInterleaved, outStride, SR_COMP_IMAG,
                      scale, false, bufferOutRe2, bufferOutIm2, outOffset, 1,
                      numB1, 0, passStr);
            passStr += "\n\t}\n";
          }
        } else {
          passStr += "\n\tif(rw)\n\t{";
          SweepRegs(SR_WRITE, fwd, outInterleaved, outStride, SR_COMP_BOTH,
                    scale, false, bufferOutRe, bufferOutIm, "outOffset", 1,
                    numB1, 0, passStr);
          passStr += "\n\t}\n";
        }
        ...
        passStr += "\n\tif(rw)\n\t{";
        SweepRegs(SR_WRITE, fwd, outInterleaved, outStride, SR_COMP_REAL, scale,
                  false, bufferOutRe, bufferOutIm, "outOffset", 1, numB1, 0,
                  passStr);
        passStr += "\n\t}\n";
        passStr += "\n\n\t__syncthreads();\n";
        passStr += "\n\tif(rw)\n\t{";
        nextPass->SweepRegs(SR_READ, fwd, outInterleaved, outStride,
        ...
        passStr += "\n\n\t__syncthreads();\n";
        passStr += "\n\tif(rw)\n\t{";
        SweepRegs(SR_WRITE, fwd, outInterleaved, outStride, SR_COMP_IMAG, scale,
                  false, bufferOutRe, bufferOutIm, "outOffset", 1, numB1, 0,
                  passStr);
        passStr += "\n\t}\n";
        passStr += "\n\n\t__syncthreads();\n";
      ...
      passStr += "\n\tif(rw)\n\t{";
      SweepRegs(SR_WRITE, fwd, outInterleaved, outStride, SR_COMP_BOTH, scale,
                false, bufferOutRe, bufferOutIm, "outOffset", 1, numB1, 0,
                passStr);
      SweepRegs(SR_WRITE, fwd, outInterleaved, outStride, SR_COMP_BOTH, scale,
                false, bufferOutRe, bufferOutIm, "outOffset", 2, numB2, numB1,
                passStr);
      SweepRegs(SR_WRITE, fwd, outInterleaved, outStride, SR_COMP_BOTH, scale,
                false, bufferOutRe, bufferOutIm, "outOffset", 4, numB4,
