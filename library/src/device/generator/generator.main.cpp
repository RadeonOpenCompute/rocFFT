...
int generate_support_size_list(std::vector<size_t> &support_size_list,
                               size_t i_upper_bound, size_t j_upper_bound,
                               size_t k_upper_bound) {
  int counter = 0;
  size_t upper_bound =
      std::max(std::max(i_upper_bound, j_upper_bound), k_upper_bound);
  ...
  */
  ...
  if (argc > 1) {
    if (strcmp(argv[1], "pow2") == 0) {
      ...
      generate_support_size_list(support_size_list, 1, 1,
      ...
      generate_support_size_list(support_size_list, 1, 2187,
                                 Large1DThreshold(rocfft_precision_single));
    } else if (strcmp(argv[1], "pow2,5") == 0) {
      // printf("Generating len pow2 and pow5 FFT kernels\n");
      generate_support_size_list(support_size_list, 3125, 1,
                                 Large1DThreshold(rocfft_precision_single));
      ...
      generate_support_size_list(support_size_list, 3125, 2187,
                                 Large1DThreshold(rocfft_precision_single));
    }
