...
struct hipfftHandle_t {
  ...
  hipfftHandle_t()
      : ip_forward(nullptr), op_forward(nullptr), ip_inverse(nullptr),
        op_inverse(nullptr), info(nullptr), workBuffer(nullptr) {}
};
...
hipfftResult hipfftPlanMany(hipfftHandle *plan, int rank, int *n, int *inembed,
  ...
  *plan = handle;
  ...
  return hipfftMakePlanMany(*plan, rank, n, inembed, istride, idist, onembed,
                            ostride, odist, type, batch, nullptr);
}

hipfftResult hipfftMakePlan_internal(hipfftHandle plan, size_t dim,
                                     size_t *lengths, hipfftType type,
                                     size_t number_of_transforms,
                                     rocfft_plan_description desc,
                                     size_t *workSize, bool dry_run) {
  size_t workBufferSize = 0;

  switch (type) {
  case HIPFFT_R2C:
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->ip_forward, rocfft_placement_inplace,
        rocfft_transform_type_real_forward, rocfft_precision_single, dim,
        lengths, number_of_transforms, desc, dry_run));
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->op_forward, rocfft_placement_notinplace,
        rocfft_transform_type_real_forward, rocfft_precision_single, dim,
        lengths, number_of_transforms, desc, dry_run));
    break;
  case HIPFFT_C2R:
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->ip_inverse, rocfft_placement_inplace,
        rocfft_transform_type_real_inverse, rocfft_precision_single, dim,
        lengths, number_of_transforms, desc, dry_run));
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->op_inverse, rocfft_placement_notinplace,
        rocfft_transform_type_real_inverse, rocfft_precision_single, dim,
        lengths, number_of_transforms, desc, dry_run));
    break;
  case HIPFFT_C2C:
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->ip_forward, rocfft_placement_inplace,
        rocfft_transform_type_complex_forward, rocfft_precision_single, dim,
        lengths, number_of_transforms, desc, dry_run));
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->op_forward, rocfft_placement_notinplace,
        rocfft_transform_type_complex_forward, rocfft_precision_single, dim,
    ...
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->ip_inverse, rocfft_placement_inplace,
        rocfft_transform_type_complex_inverse, rocfft_precision_single, dim,
        lengths, number_of_transforms, desc, dry_run));
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->op_inverse, rocfft_placement_notinplace,
        rocfft_transform_type_complex_inverse, rocfft_precision_single, dim,
  ...
  case HIPFFT_D2Z:
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->ip_forward, rocfft_placement_inplace,
        rocfft_transform_type_real_forward, rocfft_precision_double, dim,
        lengths, number_of_transforms, desc, dry_run));
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->op_forward, rocfft_placement_notinplace,
        rocfft_transform_type_real_forward, rocfft_precision_double, dim,
  ...
  case HIPFFT_Z2D:
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->ip_inverse, rocfft_placement_inplace,
        rocfft_transform_type_real_inverse, rocfft_precision_double, dim,
        lengths, number_of_transforms, desc, dry_run));
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->op_inverse, rocfft_placement_notinplace,
        rocfft_transform_type_real_inverse, rocfft_precision_double, dim,
  ...
  case HIPFFT_Z2Z:
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->ip_forward, rocfft_placement_inplace,
        rocfft_transform_type_complex_forward, rocfft_precision_double, dim,
        lengths, number_of_transforms, desc, dry_run));
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->op_forward, rocfft_placement_notinplace,
        rocfft_transform_type_complex_forward, rocfft_precision_double, dim,
    ...
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->ip_inverse, rocfft_placement_inplace,
        rocfft_transform_type_complex_inverse, rocfft_precision_double, dim,
        lengths, number_of_transforms, desc, dry_run));
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_create_internal(
        plan->op_inverse, rocfft_placement_notinplace,
        rocfft_transform_type_complex_inverse, rocfft_precision_double, dim,
...
hipfftResult hipfftMakePlan1d(hipfftHandle plan, int nx, hipfftType type,
  ...
  return hipfftMakePlan_internal(plan, 1, lengths, type, number_of_transforms,
                                 desc, workSize, false);
}
...
hipfftResult hipfftMakePlan2d(hipfftHandle plan, int nx, int ny,
  ...
  return hipfftMakePlan_internal(plan, 2, lengths, type, number_of_transforms,
                                 desc, workSize, false);
}
...
hipfftResult hipfftMakePlan3d(hipfftHandle plan, int nx, int ny, int nz,
  ...
  return hipfftMakePlan_internal(plan, 3, lengths, type, number_of_transforms,
                                 desc, workSize, false);
}
...
hipfftResult hipfftMakePlanMany(hipfftHandle plan, int rank, int *n,
  ...
  if ((inembed != nullptr) || (onembed != nullptr)) {
    ...
    // is NULL
    hipfftMakePlan_internal(plan, rank, lengths, type, number_of_transforms,
                            nullptr, workSize, true);
    ...
    ROC_FFT_CHECK_INVALID_VALUE(rocfft_plan_description_set_data_layout(
        desc, in_array_type, out_array_type, 0, 0, rank, i_strides, idist, rank,
        o_strides, odist));
  }
...
hipfftResult hipfftMakePlanMany64(hipfftHandle plan, int rank, long long int *n,
                                  ...
                                  hipfftType type, long long int batch,
                                  size_t *workSize) {
...
hipfftResult hipfftGetSize_internal(hipfftHandle plan, hipfftType type,
                                    size_t *workSize) {
...
hipfftResult hipfftGetSizeMany(hipfftHandle plan, int rank, int *n,
  ...
  hipfftHandle p;
  HIP_FFT_CHECK_AND_RETURN(hipfftPlanMany(&p, rank, n, inembed, istride, idist,
                                          onembed, ostride, odist, type,
                                          batch));
  ROC_FFT_CHECK_INVALID_VALUE(
      rocfft_plan_get_work_buffer_size(p->ip_forward, workSize));
  HIP_FFT_CHECK_AND_RETURN(hipfftDestroy(p));
...
hipfftResult hipfftExecC2C(hipfftHandle plan, hipfftComplex *idata,
  ...
  if (direction == HIPFFT_FORWARD) {
    ROC_FFT_CHECK_EXEC_FAILED(
        rocfft_execute(idata == odata ? plan->ip_forward : plan->op_forward, in,
                       out, plan->info));
  } else {
    ROC_FFT_CHECK_EXEC_FAILED(
        rocfft_execute(idata == odata ? plan->ip_inverse : plan->op_inverse, in,
                       out, plan->info));
  }
...
hipfftResult hipfftExecR2C(hipfftHandle plan, hipfftReal *idata,
  ...
  ROC_FFT_CHECK_EXEC_FAILED(
      rocfft_execute(in[0] == out[0] ? plan->ip_forward : plan->op_forward, in,
                     out, plan->info));
...
hipfftResult hipfftExecC2R(hipfftHandle plan, hipfftComplex *idata,
  ...
  ROC_FFT_CHECK_EXEC_FAILED(
      rocfft_execute(in[0] == out[0] ? plan->ip_inverse : plan->op_inverse, in,
                     out, plan->info));
...
hipfftResult hipfftGetVersion(int *version) {
  ...
  for (size_t i = 0; i < sections.size(); i++) {
    ...
    boost::split(sl, sections[i],
                 boost::is_any_of("-")); // remove potential git tag string
    if (sl[0].size() == 0)
      result << "00";
    else if (sl[0].size() == 1)
      result << "0" << sl[0][0];
