...
class RefLibOp {
    ...
    void DataSetup(const void* data_p)
    {
        ...
        } else if ((data->node->scheme == CS_KERNEL_FFT_MUL) || (data->node->scheme == CS_KERNEL_PAD_MUL) || (data->node->scheme == CS_KERNEL_RES_MUL)) {
...
public:
    ...
    void VerifyResult(const void* data_p)
    {
        ...
        if (data->node->scheme == CS_KERNEL_CHIRP) {
            out_size_bytes *= 2;
        } else if ((data->node->scheme == CS_KERNEL_FFT_MUL) || (data->node->scheme == CS_KERNEL_PAD_MUL)) {
