...
typedef void *(*ftype_fftwf_plan_many_dft)(int rank, const int *n, int howmany,
                                           ...
                                           int idist, local_fftwf_complex *out,
...
class RefLibHandle {
  RefLibHandle() : fftw3f_lib(nullptr), fftw3_lib(nullptr) {
    ...
    if (!env_value_fftw3f) {
      std::cout
          << "error finding fftw3f lib, set env variable ROCFFT_DBG_FFTW3F_LIB"
          << std::endl;
    }

    if (!env_value_fftw3) {
      std::cout
          << "error finding fftw3 lib, set env variable ROCFFT_DBG_FFTW3_LIB"
          << std::endl;
    }
...
public:
  ...
  RefLibHandle &
  operator=(const RefLibHandle &) = delete; // prohibit assignment operator
...
class RefLibOp {
  ...
  void CopyVector(local_fftwf_complex *dst, local_fftwf_complex *src,
                  size_t batch, size_t dist, std::vector<size_t> length,
  ...
  void CopyInputVector(const void *data_p, size_t offset = 0) {
    ...
    size_t in_size_bytes =
        (data->node->iDist * data->node->batch) * sizeof(float);
    ...
    CopyVector(in, tmp_mem, data->node->batch, data->node->iDist,
               data->node->length, data->node->inStride);

    if (tmp_mem)
      free(tmp_mem);
  }

  inline float2 TwMul(float2 *twiddles, const size_t twl, const int direction,
                      float2 val, size_t u) {
    size_t j = u & 255;
    ...
    do {
      u >>= 8;
      j = u & 255;
      real = (result.x * twiddles[256 * h + j].x -
              result.y * twiddles[256 * h + j].y);
      imag = (result.y * twiddles[256 * h + j].x +
              result.x * twiddles[256 * h + j].y);
      result.x = real;
      result.y = imag;
      h++;
    } while (h < twl);
  ...
  inline void chirp_fft(size_t N, size_t M, int dir, local_fftwf_complex *vec) {
    ...
    ftype_fftwf_plan_many_dft local_fftwf_plan_many_dft =
        (ftype_fftwf_plan_many_dft)dlsym(refHandle.fftw3f_lib,
                                         "fftwf_plan_many_dft");
    ...
    ftype_fftwf_destroy_plan local_fftwf_destroy_plan =
        (ftype_fftwf_destroy_plan)dlsym(refHandle.fftw3f_lib,
                                        "fftwf_destroy_plan");
    ...
    void *p;
    if (dir == -1)
      p = local_fftwf_plan_many_dft(1, n, 1, vec, NULL, 1, n[0], vec, NULL, 1,
                                    n[0], LOCAL_FFTW_FORWARD,
                                    LOCAL_FFTW_ESTIMATE);
    else
      p = local_fftwf_plan_many_dft(1, n, 1, vec, NULL, 1, n[0], vec, NULL, 1,
                                    n[0], LOCAL_FFTW_BACKWARD,
                                    LOCAL_FFTW_ESTIMATE);
  ...
  void Execute(const void *data_p) {
    ...
    if (data->node->scheme == CS_KERNEL_STOCKHAM) {
      ...
      if (data->node->direction == -1)
        p = local_fftwf_plan_many_dft(1, n, howmany, in, NULL, 1, n[0], ot,
                                      NULL, 1, n[0], LOCAL_FFTW_FORWARD,
                                      LOCAL_FFTW_ESTIMATE);
      else
        p = local_fftwf_plan_many_dft(1, n, howmany, in, NULL, 1, n[0], ot,
                                      NULL, 1, n[0], LOCAL_FFTW_BACKWARD,
                                      LOCAL_FFTW_ESTIMATE);
      ...
      // [N/2 + 1] elements
      size_t in_size_bytes =
          (data->node->iDist * data->node->batch) * 2 * sizeof(float);

      local_fftwf_complex *tmp_mem =
          (local_fftwf_complex *)malloc(in_size_bytes);
      hipMemcpy(tmp_mem, data->bufIn[0], in_size_bytes, hipMemcpyDeviceToHost);
      ...
      for (size_t b = 0; b < data->node->batch; b++) {
        ...
        {
          ot[data->node->oDist * b + i][0] =
              tmp_mem[data->node->iDist * b + i][0];
          ot[data->node->oDist * b + i][1] =
              tmp_mem[data->node->iDist * b + i][1];
        }
      }
...
public:
  RefLibOp(const void *data_p)
      : in(nullptr), ot(nullptr), lb(nullptr), totalSize(0) {
    DataSetup(data_p);
    Execute(data_p);
  }
  ...
  void VerifyResult(const void *data_p) {
    ...
    size_t out_size_bytes =
        (data->node->oDist * data->node->batch) * sizeof(float);
    ...
    if (data->node->scheme == CS_KERNEL_CHIRP) {
      out_size_bytes *= 2;
               ...
               (data->node->scheme == CS_KERNEL_PAD_MUL)) {
      bufOut =
          ((char *)bufOut + 2 * 2 * sizeof(float) * data->node->lengthBlue);
    }
    ...
    local_fftwf_complex *tmp_mem =
        (local_fftwf_complex *)malloc(out_size_bytes);
    ...
    if (data->node->scheme == CS_KERNEL_TRANSPOSE) {
      ...
      CopyVector(lb, tmp_mem, data->node->batch, data->node->oDist,
                 length_transpose_output, data->node->outStride);
    } else if (data->node->scheme == CS_KERNEL_CHIRP) {
      std::vector<size_t> length_chirp;
      length_chirp.push_back(data->node->lengthBlue);
      CopyVector(lb, tmp_mem, 2 * data->node->batch, data->node->oDist,
                 length_chirp, data->node->outStride);
    } else if (data->node->scheme == CS_KERNEL_PAD_MUL) {
      std::vector<size_t> length_ot;
      length_ot.push_back(data->node->lengthBlue);
      for (size_t i = 1; i < data->node->length.size(); i++)
        length_ot.push_back(data->node->length[i]);
      CopyVector(lb, tmp_mem, data->node->batch, data->node->oDist, length_ot,
                 data->node->outStride);
    } else {
      CopyVector(lb, tmp_mem, data->node->batch, data->node->oDist,
                 data->node->length, data->node->outStride);
    }
