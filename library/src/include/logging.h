...
protected:
   ...
   ************************************************************************************/
  // Default output
  template <typename T> static void print_value(std::ostream &os, const T &x) {
    os << x;
  }
  ...
  struct print_tuple_recurse {
    template <typename F>
    __attribute__((always_inline)) void operator()(F &print_argument,
                                                   const T &tuple) {
  ...
  template <typename T> struct print_tuple_recurse<T, 0> {
    template <typename F>
    __attribute__((always_inline)) void operator()(F &print_argument,
                                                   const T &tuple) {}
  };
  ...
  template <typename T>
  static typename std::enable_if<std::is_enum<T>{}, size_t>::type
  hash(const T &x) {
  ...
  struct tuple_hash_recurse {
    __attribute__((always_inline)) size_t operator()(const TUP &tup) {
      ...
      return seed ^ (hash(std::get<idx - 1>(tup)) + 0x9e3779b9 + (seed << 6) +
                     (seed >> 2));
    }
  };

  // Leaf node
  template <typename TUP> struct tuple_hash_recurse<TUP, 0> {
    __attribute__((always_inline)) size_t operator()(const TUP &) { return 0; }
  };

  // Hash function class compatible with STL containers
  template <typename TUP> struct hash_t {
    static_assert(std::tuple_size<TUP>{} % 2 == 0, "Tuple size must be even");
   ...
   ************************************************************************************/
  template <typename T> static bool equal(const T &x1, const T &x2) {
...
public:
  ...
  static void log_arguments(std::ostream &os, const char *sep, H head,
                            Ts &&... xs) {
    os << head;
 ...
 ************************************************************************************/
template <typename TUP> class argument_profile : tuple_helper {
  // Output stream
  std::ostream &os;
...
template <typename... Ts> inline void log_trace(Ts &&... xs) {
  if (LOG_TRACE_ENABLED())
    tuple_helper::log_arguments(*LogSingleton::GetInstance().GetTraceOS(), ",",
                                std::forward<Ts>(xs)...);
}
...
template <typename... Ts> inline void log_bench(Ts &&... xs) {
  if (LOG_BENCH_ENABLED())
    tuple_helper::log_arguments(*LogSingleton::GetInstance().GetBenchOS(), " ",
                                std::forward<Ts>(xs)...);
}
...
inline void log_profile(const char *func, Ts &&... xs) {
  if (LOG_PROFILE_ENABLED()) {
    auto tup =
        std::make_tuple("rocfft_function", func, std::forward<Ts>(xs)...);
    static argument_profile<decltype(tup)> profile(
