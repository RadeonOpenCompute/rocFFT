...
private:
  ...
  TreeNode(TreeNode *p)
      : parent(p), scheme(CS_NONE), obIn(OB_UNINIT), obOut(OB_UNINIT),
        large1D(0), lengthBlue(0), iOffset(0), oOffset(0),
...
public:
  ...
  TreeNode &
  operator=(const TreeNode &) = delete; // disallow assignment operator
  ...
  static void DeleteNode(TreeNode *node) {
    ...
    for (children_p = node->childNodes.begin();
         children_p != node->childNodes.end(); children_p++)
  ...
  void TraverseTreeAssignPlacementsLogicA(rocfft_array_type rootIn,
  ...
  void TraverseTreeCollectLeafsLogicA(std::vector<TreeNode *> &seq,
                                      ...
                                      size_t &blueSize, size_t &chirpSize);
...
struct ExecPlan {
  ...
  ExecPlan()
      : rootPlan(nullptr), workBufSize(0), tmpWorkBufSize(0),
        copyWorkBufSize(0), blueWorkBufSize(0) {}
};
