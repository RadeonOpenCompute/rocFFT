...
#define POWX_SMALL_GENERATOR(FUNCTION_NAME, IP_FWD_KERN_NAME,                  \
                             IP_BACK_KERN_NAME, OP_FWD_KERN_NAME,              \
                             OP_BACK_KERN_NAME, PRECISION)                     \
    ...
    if (data->node->placement == rocfft_placement_inplace) {                   \
      if (data->node->inStride[0] == 1 && data->node->outStride[0] == 1) {     \
        if (data->node->direction == -1) {                                     \
          hipLaunchKernelGGL(                                                  \
              HIP_KERNEL_NAME(IP_FWD_KERN_NAME<PRECISION, SB_UNIT>),           \
              dim3(data->gridParam.b_x), dim3(data->gridParam.tpb_x), 0,       \
              rocfft_stream, (PRECISION *)data->node->twiddles,                \
              data->node->length.size(), data->node->devKernArg,               \
              data->node->devKernArg + 1 * KERN_ARGS_ARRAY_WIDTH,              \
              data->node->batch, (PRECISION *)data->bufIn[0]);                 \
        } else {                                                               \
          hipLaunchKernelGGL(                                                  \
              HIP_KERNEL_NAME(IP_BACK_KERN_NAME<PRECISION, SB_UNIT>),          \
              dim3(data->gridParam.b_x), dim3(data->gridParam.tpb_x), 0,       \
              rocfft_stream, (PRECISION *)data->node->twiddles,                \
              data->node->length.size(), data->node->devKernArg,               \
              data->node->devKernArg + 1 * KERN_ARGS_ARRAY_WIDTH,              \
...
#define POWX_LARGE_SBCC_GENERATOR(FUNCTION_NAME, FWD_KERN_NAME,                \
                                  BACK_KERN_NAME, PRECISION)                   \
  void FUNCTION_NAME(const void *data_p, void *back_p) {                       \
    DeviceCallIn *data = (DeviceCallIn *)data_p;                               \
    hipStream_t rocfft_stream = data->rocfft_stream;                           \
    if (data->node->direction == -1) {                                         \
      hipLaunchKernelGGL(HIP_KERNEL_NAME(FWD_KERN_NAME<PRECISION, SB_UNIT>),   \
                         ...
                         dim3(data->gridParam.tpb_x), 0, rocfft_stream,        \
...
#define POWX_LARGE_SBRC_GENERATOR(FUNCTION_NAME, FWD_KERN_NAME,                \
                                  BACK_KERN_NAME, PRECISION)                   \
  void FUNCTION_NAME(const void *data_p, void *back_p) {                       \
    DeviceCallIn *data = (DeviceCallIn *)data_p;                               \
    hipStream_t rocfft_stream = data->rocfft_stream;                           \
