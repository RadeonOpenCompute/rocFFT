...
inline void GetBlockComputeTable(size_t N, size_t &bwd, size_t &wgs,
                                 size_t &lds) {
  switch (N) {
  case 256:
    bwd = 8;
...
inline void DetermineSizes(const size_t &length, size_t &workGroupSize,
                           size_t &numTrans) {
  ...
  } else if (primeFactorsExpanded[13] == length) // Length is pure power of 13
  {
    ...
    size_t maxWorkGroupSize =
        MAX_WORK_GROUP_SIZE; // maximum work group size desired
