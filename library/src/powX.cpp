...
void PlanPowX(ExecPlan& execPlan)
{
    for (size_t i = 0; i < execPlan.execSeq.size(); i++) {
        if ((execPlan.execSeq[i]->scheme == CS_KERNEL_STOCKHAM) || (execPlan.execSeq[i]->scheme == CS_KERNEL_STOCKHAM_BLOCK_CC) || (execPlan.execSeq[i]->scheme == CS_KERNEL_STOCKHAM_BLOCK_RC)) {
    ...
    if (execPlan.execSeq[0]->precision == rocfft_precision_single) {
        ...
        for (size_t i = 0; i < execPlan.execSeq.size(); i++) {
            ...
            if (execPlan.execSeq[i]->scheme == CS_KERNEL_STOCKHAM) {
                ...
                gp.tpb_x = wgs;
            } else if ((execPlan.execSeq[i]->scheme == CS_KERNEL_TRANSPOSE) || (execPlan.execSeq[i]->scheme == CS_KERNEL_TRANSPOSE_XY_Z) || (execPlan.execSeq[i]->scheme == CS_KERNEL_TRANSPOSE_Z_XY)) {
...
void TransformPowX(const ExecPlan& execPlan, void* in_buffer[], void* out_buffer[], rocfft_execution_info info)
{
    ...
    for (size_t i = 0; i < execPlan.execSeq.size(); i++) //multiple kernels involving transpose
    {
        ...
        case OB_TEMP_BLUESTEIN:
            data.bufIn[0] = (void*)((char*)info->workBuffer + (execPlan.tmpWorkBufSize + execPlan.copyWorkBufSize + data.node->iOffset) * inBytes);
            break;
        default:
            assert(false);
        }
        ...
        case OB_TEMP_CMPLX_FOR_REAL:
            ...
            break;
        case OB_TEMP_BLUESTEIN:
            data.bufOut[0] = (void*)((char*)info->workBuffer + (execPlan.tmpWorkBufSize + execPlan.copyWorkBufSize + data.node->oOffset) * inBytes);
            break;
        default:
            assert(false);
        }
