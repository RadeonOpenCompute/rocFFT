...
void PlanPowX(ExecPlan &execPlan) {
  for (size_t i = 0; i < execPlan.execSeq.size(); i++) {
    if ((execPlan.execSeq[i]->scheme == CS_KERNEL_STOCKHAM) ||
      ...
      execPlan.execSeq[i]->twiddles =
          twiddles_create(execPlan.execSeq[i]->length[0],
                          execPlan.execSeq[i]->precision, false);
    }
  ...
  for (size_t i = 0; i < execPlan.execSeq.size(); i++) {
    execPlan.execSeq[i]->devKernArg =
        kargs_create(execPlan.execSeq[i]->length, execPlan.execSeq[i]->inStride,
                     execPlan.execSeq[i]->outStride, execPlan.execSeq[i]->iDist,
  ...
  if (execPlan.execSeq[0]->precision == rocfft_precision_single) {
    ...
    for (size_t i = 0; i < execPlan.execSeq.size(); i++) {
      ...
      if (execPlan.execSeq[i]->scheme == CS_KERNEL_STOCKHAM) {
        ...
        GetWGSAndNT(
            execPlan.execSeq[i]->length[0], workGroupSize,
    ...
    for (size_t i = 0; i < execPlan.execSeq.size(); i++) {
      ...
      if (execPlan.execSeq[i]->scheme == CS_KERNEL_STOCKHAM) {
        ...
        ptr = function_pool::get_function_double(std::make_pair(
            execPlan.execSeq[i]->length[0], CS_KERNEL_STOCKHAM_BLOCK_CC));
...
void TransformPowX(const ExecPlan &execPlan, void *in_buffer[],
  ...
  for (size_t i = 0; i < execPlan.execSeq.size();
       i++) // multiple kernels involving transpose
  {
    DeviceCallIn data;
    DeviceCallOut back;
    ...
    case OB_TEMP_CMPLX_FOR_REAL:
      data.bufIn[0] = (void *)((char *)info->workBuffer +
                               execPlan.tmpWorkBufSize * inBytes);
      break;
    case OB_TEMP_BLUESTEIN:
      data.bufIn[0] = (void *)((char *)info->workBuffer +
                               (execPlan.tmpWorkBufSize +
                                execPlan.copyWorkBufSize + data.node->iOffset) *
                                   inBytes);
      break;
    default:
      assert(false);
