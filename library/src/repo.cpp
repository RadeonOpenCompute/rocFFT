...
void Repo::CreatePlan(rocfft_plan plan) {
  ...
  std::map<rocfft_plan_t, ExecPlan>::const_iterator it =
      repo.planUnique.find(*plan);
  if (it == repo.planUnique.end()) // if not found
  {
    ...
    repo.planUnique[*plan] =
        execPlan; // add this plan into member planUnique (type of map)
    repo.execLookup[plan] =
        execPlan; // add this plan into member execLookup (type of map)
  } else          // find the stored plan
  {
    repo.execLookup[plan] =
        it->second; // retrieve this plan and put it into member execLookup
  }
}
